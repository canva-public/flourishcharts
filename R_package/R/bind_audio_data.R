...
bind_audio_data <- function(
  ...
  if (!is.null(url_output_format) && !is.null(url_input_format)) {
    if (isTRUE(grepl("^%", url_output_format))) {
      ...
      new_list$x$metadata$data$url$type_id <- paste0(
        "datetime$",
        url_input_format
      )
      new_list$x$metadata$data$url$output_format_id <- paste0(
        "datetime$",
        url_output_format
      )
      ...
      isTRUE(
        !is.na(url_output_format) &&
          isFALSE(grepl("_", url_output_format)) &&
          isFALSE(grepl("^%", url_output_format))
      )
    ) {
