...
bind_bar_chart_race_data <- function(
  ...
  if (!is.null(label_output_format) && !is.null(label_input_format)) {
    if (isTRUE(grepl("^%", label_output_format))) {
      ...
      new_list$x$metadata$data$label$type_id <- paste0(
        "datetime$",
        label_input_format
      )
      new_list$x$metadata$data$label$output_format_id <- paste0(
        "datetime$",
        label_output_format
      )
      ...
      isTRUE(
        !is.na(label_output_format) &&
          isFALSE(grepl("_", label_output_format)) &&
          isFALSE(grepl("^%", label_output_format))
      )
    ) {
  ...
  if (!is.null(category_output_format) && !is.null(category_input_format)) {
    if (isTRUE(grepl("^%", category_output_format))) {
      ...
      new_list$x$metadata$data$category$type_id <- paste0(
        "datetime$",
        category_input_format
      )
      new_list$x$metadata$data$category$output_format_id <- paste0(
        "datetime$",
      ...
      isTRUE(
        !is.na(category_output_format) &&
          isFALSE(grepl("_", category_output_format)) &&
          isFALSE(grepl("^%", category_output_format))
      )
    ) {
  ...
  if (!is.null(image_output_format) && !is.null(image_input_format)) {
    if (isTRUE(grepl("^%", image_output_format))) {
      ...
      new_list$x$metadata$data$image$type_id <- paste0(
        "datetime$",
        image_input_format
      )
      new_list$x$metadata$data$image$output_format_id <- paste0(
        "datetime$",
        image_output_format
      )
      ...
      isTRUE(
        !is.na(image_output_format) &&
          isFALSE(grepl("_", image_output_format)) &&
          isFALSE(grepl("^%", image_output_format))
      )
    ) {
