...
bind_chord_diagram_data <- function(
  ...
  if (!is.null(connections_data)) {
    columns_connections_data <- c(
      paste(source),
      paste(target),
      paste(value),
      NULL
    )
    columns_connections_data <- columns_connections_data[
      !sapply(columns_connections_data, is.null)
    ]
    ...
    connections_data[, int_columns_connections_data] <- lapply(
      connections_data[, int_columns_connections_data],
      as.character
    )
  }
