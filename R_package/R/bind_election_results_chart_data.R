...
bind_election_results_chart_data <- function(
  ...
  if (!is.null(values_now_data)) {
    columns_values_now_data <- c(
      paste(values_now_name),
      paste(values_now_values),
      NULL
    )
    columns_values_now_data <- columns_values_now_data[
      !sapply(columns_values_now_data, is.null)
    ]
    ...
    values_now_data[, int_columns_values_now_data] <- lapply(
      values_now_data[, int_columns_values_now_data],
      as.character
    )
  }
  if (!is.null(values_historical_data)) {
    columns_values_historical_data <- c(
      paste(values_historical_name),
      paste(values_historical_values),
      NULL
    )
    columns_values_historical_data <- columns_values_historical_data[
      !sapply(columns_values_historical_data, is.null)
    ]
    spelling_check_column_names(
      strings = strsplit(
        columns_values_historical_data,
        split = ",",
        fixed = TRUE
      ),
      data = values_historical_data
    )
    int_columns_values_historical_data <- sapply(
      values_historical_data,
      is.integer
    )
    values_historical_data[, int_columns_values_historical_data] <- lapply(
