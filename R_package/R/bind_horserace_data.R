...
bind_horserace_data <- function(
  ...
  if (!is.null(horserace_data)) {
    columns_horserace_data <- c(
      paste(horserace_name),
      paste(horserace_stages),
      paste(horserace_category),
      paste(horserace_pic),
      paste(horserace_filter),
      NULL
    )
    columns_horserace_data <- columns_horserace_data[
      !sapply(columns_horserace_data, is.null)
    ]
    ...
    horserace_data[, int_columns_horserace_data] <- lapply(
      horserace_data[, int_columns_horserace_data],
      as.character
    )
  }
  if (!is.null(captions_data)) {
    columns_captions_data <- c(
      paste(captions_from),
      paste(captions_to),
      paste(captions_text),
      paste(captions_image),
      NULL
    )
    columns_captions_data <- columns_captions_data[
      !sapply(columns_captions_data, is.null)
    ]
    spelling_check_column_names(
    ...
    captions_data[, int_columns_captions_data] <- lapply(
      captions_data[, int_columns_captions_data],
      as.character
    )
  }
  ...
  if (
    !is.null(horserace_name_output_format) &&
      !is.null(horserace_name_input_format)
  ) {
    if (isTRUE(grepl("^%", horserace_name_output_format))) {
      new_list$x$metadata$horserace$name$type <- "datetime"
      new_list$x$metadata$horserace$name$type_id <- paste0(
        "datetime$",
        horserace_name_input_format
      )
      new_list$x$metadata$horserace$name$output_format_id <- paste0(
        "datetime$",
        horserace_name_output_format
      )
      ...
      isTRUE(
        !is.na(horserace_name_output_format) &&
          isFALSE(grepl("_", horserace_name_output_format)) &&
          isFALSE(grepl("^%", horserace_name_output_format))
      )
    ) {
    ...
    if (
      is.na(horserace_name_input_format) && !is.na(horserace_name_output_format)
    ) {
  ...
  if (
    !is.null(horserace_stages_output_format) &&
      !is.null(horserace_stages_input_format)
  ) {
    if (isTRUE(grepl("^%", horserace_stages_output_format))) {
      new_list$x$metadata$horserace$stages$type <- "datetime"
      new_list$x$metadata$horserace$stages$type_id <- paste0(
        "datetime$",
        horserace_stages_input_format
      )
      new_list$x$metadata$horserace$stages$output_format_id <- paste0(
        "datetime$",
        horserace_stages_output_format
      )
      ...
      isTRUE(
        !is.na(horserace_stages_output_format) &&
          isFALSE(grepl("_", horserace_stages_output_format)) &&
          isFALSE(grepl("^%", horserace_stages_output_format))
      )
    ) {
    ...
    if (
      is.na(horserace_stages_input_format) &&
        !is.na(horserace_stages_output_format)
    ) {
  ...
  if (
    !is.null(horserace_category_output_format) &&
      !is.null(horserace_category_input_format)
  ) {
    if (isTRUE(grepl("^%", horserace_category_output_format))) {
      new_list$x$metadata$horserace$category$type <- "datetime"
      new_list$x$metadata$horserace$category$type_id <- paste0(
        "datetime$",
        horserace_category_input_format
      )
      new_list$x$metadata$horserace$category$output_format_id <- paste0(
        "datetime$",
        horserace_category_output_format
      )
      ...
      isTRUE(
        !is.na(horserace_category_output_format) &&
          isFALSE(grepl("_", horserace_category_output_format)) &&
          isFALSE(grepl("^%", horserace_category_output_format))
      )
    ) {
    ...
    if (
      is.na(horserace_category_input_format) &&
        !is.na(horserace_category_output_format)
    ) {
  ...
  if (
    !is.null(horserace_pic_output_format) &&
      !is.null(horserace_pic_input_format)
  ) {
    if (isTRUE(grepl("^%", horserace_pic_output_format))) {
      new_list$x$metadata$horserace$pic$type <- "datetime"
      new_list$x$metadata$horserace$pic$type_id <- paste0(
        "datetime$",
        horserace_pic_input_format
      )
      new_list$x$metadata$horserace$pic$output_format_id <- paste0(
        "datetime$",
        horserace_pic_output_format
      )
      ...
      isTRUE(
        !is.na(horserace_pic_output_format) &&
          isFALSE(grepl("_", horserace_pic_output_format)) &&
          isFALSE(grepl("^%", horserace_pic_output_format))
      )
    ) {
    ...
    if (
      is.na(horserace_pic_input_format) && !is.na(horserace_pic_output_format)
    ) {
  ...
  if (
    !is.null(horserace_filter_output_format) &&
      !is.null(horserace_filter_input_format)
  ) {
    if (isTRUE(grepl("^%", horserace_filter_output_format))) {
      new_list$x$metadata$horserace$filter$type <- "datetime"
      new_list$x$metadata$horserace$filter$type_id <- paste0(
        "datetime$",
        horserace_filter_input_format
      )
      new_list$x$metadata$horserace$filter$output_format_id <- paste0(
        "datetime$",
        horserace_filter_output_format
      )
      ...
      isTRUE(
        !is.na(horserace_filter_output_format) &&
          isFALSE(grepl("_", horserace_filter_output_format)) &&
          isFALSE(grepl("^%", horserace_filter_output_format))
      )
    ) {
    ...
    if (
      is.na(horserace_filter_input_format) &&
        !is.na(horserace_filter_output_format)
    ) {
  ...
  if (
    !is.null(captions_from_output_format) &&
      !is.null(captions_from_input_format)
  ) {
    if (isTRUE(grepl("^%", captions_from_output_format))) {
      new_list$x$metadata$captions$from$type <- "datetime"
      new_list$x$metadata$captions$from$type_id <- paste0(
        "datetime$",
        captions_from_input_format
      )
      new_list$x$metadata$captions$from$output_format_id <- paste0(
        "datetime$",
        captions_from_output_format
      )
      ...
      isTRUE(
        !is.na(captions_from_output_format) &&
          isFALSE(grepl("_", captions_from_output_format)) &&
          isFALSE(grepl("^%", captions_from_output_format))
      )
    ) {
    ...
    if (
      is.na(captions_from_input_format) && !is.na(captions_from_output_format)
    ) {
  ...
  if (
    !is.null(captions_to_output_format) && !is.null(captions_to_input_format)
  ) {
    if (isTRUE(grepl("^%", captions_to_output_format))) {
      new_list$x$metadata$captions$to$type <- "datetime"
      new_list$x$metadata$captions$to$type_id <- paste0(
        "datetime$",
        captions_to_input_format
      )
      new_list$x$metadata$captions$to$output_format_id <- paste0(
        "datetime$",
        captions_to_output_format
      )
      ...
      isTRUE(
        !is.na(captions_to_output_format) &&
          isFALSE(grepl("_", captions_to_output_format)) &&
          isFALSE(grepl("^%", captions_to_output_format))
      )
    ) {
  ...
  if (
    !is.null(captions_text_output_format) &&
      !is.null(captions_text_input_format)
  ) {
    if (isTRUE(grepl("^%", captions_text_output_format))) {
      new_list$x$metadata$captions$text$type <- "datetime"
      new_list$x$metadata$captions$text$type_id <- paste0(
        "datetime$",
        captions_text_input_format
      )
      new_list$x$metadata$captions$text$output_format_id <- paste0(
        "datetime$",
        captions_text_output_format
      )
      ...
      isTRUE(
        !is.na(captions_text_output_format) &&
          isFALSE(grepl("_", captions_text_output_format)) &&
          isFALSE(grepl("^%", captions_text_output_format))
      )
    ) {
    ...
    if (
      is.na(captions_text_input_format) && !is.na(captions_text_output_format)
    ) {
  ...
  if (
    !is.null(captions_image_output_format) &&
      !is.null(captions_image_input_format)
  ) {
    if (isTRUE(grepl("^%", captions_image_output_format))) {
      new_list$x$metadata$captions$image$type <- "datetime"
      new_list$x$metadata$captions$image$type_id <- paste0(
        "datetime$",
        captions_image_input_format
      )
      new_list$x$metadata$captions$image$output_format_id <- paste0(
        "datetime$",
        captions_image_output_format
      )
      ...
      isTRUE(
        !is.na(captions_image_output_format) &&
          isFALSE(grepl("_", captions_image_output_format)) &&
          isFALSE(grepl("^%", captions_image_output_format))
      )
    ) {
    ...
    if (
      is.na(captions_image_input_format) && !is.na(captions_image_output_format)
    ) {
