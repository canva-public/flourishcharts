...
bind_marimekko_data <- function(
  ...
  if (!is.null(data)) {
    columns_data <- c(
      paste(name),
      paste(unstacked_value),
      paste(stacked_values),
      paste(facet),
      paste(category),
      paste(filter),
      paste(metadata),
  ...
  if (
    !is.null(unstacked_value_output_format) &&
      !is.null(unstacked_value_input_format)
  ) {
    if (isTRUE(grepl("^%", unstacked_value_output_format))) {
      new_list$x$metadata$data$unstacked_value$type <- "datetime"
      new_list$x$metadata$data$unstacked_value$type_id <- paste0(
        "datetime$",
        unstacked_value_input_format
      )
      new_list$x$metadata$data$unstacked_value$output_format_id <- paste0(
        "datetime$",
        unstacked_value_output_format
      )
      ...
      isTRUE(
        !is.na(unstacked_value_output_format) &&
          isFALSE(grepl("_", unstacked_value_output_format)) &&
          isFALSE(grepl("^%", unstacked_value_output_format))
      )
    ) {
    ...
    if (
      is.na(unstacked_value_input_format) &&
        !is.na(unstacked_value_output_format)
    ) {
  ...
  if (
    !is.null(stacked_values_output_format) &&
      !is.null(stacked_values_input_format)
  ) {
    if (isTRUE(grepl("^%", stacked_values_output_format))) {
      new_list$x$metadata$data$stacked_values$type <- "datetime"
      new_list$x$metadata$data$stacked_values$type_id <- paste0(
        "datetime$",
        stacked_values_input_format
      )
      new_list$x$metadata$data$stacked_values$output_format_id <- paste0(
        "datetime$",
        stacked_values_output_format
      )
      ...
      isTRUE(
        !is.na(stacked_values_output_format) &&
          isFALSE(grepl("_", stacked_values_output_format)) &&
          isFALSE(grepl("^%", stacked_values_output_format))
      )
    ) {
    ...
    if (
      is.na(stacked_values_input_format) && !is.na(stacked_values_output_format)
    ) {
