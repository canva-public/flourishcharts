...
bind_network_graph_data <- function(
  ...
  if (!is.null(links_data)) {
    columns_links_data <- c(
      paste(links_source),
      paste(links_target),
      paste(links_value),
      NULL
    )
    columns_links_data <- columns_links_data[
      !sapply(columns_links_data, is.null)
    ]
    ...
    links_data[, int_columns_links_data] <- lapply(
      links_data[, int_columns_links_data],
      as.character
    )
  }
  if (!is.null(nodes_data)) {
    columns_nodes_data <- c(
      paste(nodes_id),
      paste(nodes_group),
      paste(nodes_size),
      paste(nodes_image),
      paste(nodes_metadata),
      NULL
    )
    columns_nodes_data <- columns_nodes_data[
      !sapply(columns_nodes_data, is.null)
    ]
    spelling_check_column_names(
    ...
    nodes_data[, int_columns_nodes_data] <- lapply(
      nodes_data[, int_columns_nodes_data],
      as.character
    )
  }
