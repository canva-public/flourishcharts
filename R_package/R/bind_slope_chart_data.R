...
bind_slope_chart_data <- function(
  ...
  if (!is.null(id_output_format) && !is.null(id_input_format)) {
    if (isTRUE(grepl("^%", id_output_format))) {
      ...
      new_list$x$metadata$data$id$type_id <- paste0(
        "datetime$",
        id_input_format
      )
      new_list$x$metadata$data$id$output_format_id <- paste0(
        "datetime$",
        id_output_format
      )
  ...
  if (
    !is.null(line_thickness_output_format) &&
    ...
    if (isTRUE(grepl("^%", line_thickness_output_format))) {
      ...
      new_list$x$metadata$data$line_thickness$type_id <- paste0(
        "datetime$",
        line_thickness_input_format
      )
      new_list$x$metadata$data$line_thickness$output_format_id <- paste0(
        "datetime$",
        line_thickness_output_format
      )
      ...
      isTRUE(
        !is.na(line_thickness_output_format) &&
          isFALSE(grepl("_", line_thickness_output_format)) &&
          isFALSE(grepl("^%", line_thickness_output_format))
      )
    ) {
    ...
    if (
      is.na(line_thickness_input_format) && !is.na(line_thickness_output_format)
    ) {
