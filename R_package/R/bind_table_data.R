...
bind_table_data <- function(
  ...
  if (!is.null(rows_data)) {
    ...
    rows_data[, int_columns_rows_data] <- lapply(
      rows_data[, int_columns_rows_data],
      as.character
    )
  }
  ...
  if (!is.null(columns_output_format) && !is.null(columns_input_format)) {
    if (isTRUE(grepl("^%", columns_output_format))) {
      ...
      new_list$x$metadata$rows$columns$type_id <- paste0(
        "datetime$",
        columns_input_format
      )
      new_list$x$metadata$rows$columns$output_format_id <- paste0(
        "datetime$",
      ...
      isTRUE(
        !is.na(columns_output_format) &&
          isFALSE(grepl("_", columns_output_format)) &&
          isFALSE(grepl("^%", columns_output_format))
      )
    ) {
