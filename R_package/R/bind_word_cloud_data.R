...
bind_word_cloud_data <- function(
  ...
  if (!is.null(words_data)) {
    ...
    columns_words_data <- columns_words_data[
      !sapply(columns_words_data, is.null)
    ]
    ...
    words_data[, int_columns_words_data] <- lapply(
      words_data[, int_columns_words_data],
      as.character
    )
  }
