...
#' @param chart_description Optional - a screen-reader description. A text alternative to the visual content that will only be visible to screen-readers, e.g. <U+201C>The line chart shows China consistently higher than the other countries since 1990<U+201D>. Do no replicate your title, since that will also be read by screenreaders.
...
flourish <- function(
  ...
  api_version <- "5.4.2"
  templates_chart_type_data <- templates_private[
    templates_private$chart_type == chart_type,
  ]
  if (!is.null(base_visualisation_id)) {
    ...
    base_visualisation_url <- sprintf(
      "https://public.flourish.studio/visualisation/%d/visualisation-object.json",
      base_visualisation_id
    )
    resp <- httr::GET(base_visualisation_url)
    if (resp$status_code == 403) {
      stop(paste(
        "Access denied accessing base visualisation ",
        base_visualisation_url,
        " - please ensure it is published for public use"
      ))
    }
    ...
    for (i in seq_along(jsonRespParsed$data)) {
      ...
      for (col_name in names(jsonRespParsed$data[[i]])) {
        if (list_element[[col_name]]$type == "string") {
          jsonRespParsed$data[[i]][[col_name]][
            jsonRespParsed$data[[i]][[col_name]] == "NA"
          ] <- ""
          jsonRespParsed$data[[i]][[
            col_name
          ]] <- as.character(jsonRespParsed$data[[i]][[col_name]])
          ...
          jsonRespParsed$data[[i]][[
            col_name
          ]] <- suppressWarnings(as.numeric(jsonRespParsed$data[[i]][[
            col_name
          ]]))
    ...
    if (
      jsonRespParsed$template %in%
        c("@flourish/line-bar-pie", "@flourish/hierarchy")
    ) {
      base_viz_chart_type <- base_coalesce_chart_type(
        jsonRespParsed$template,
        jsonRespParsed$state$chart_type,
        jsonRespParsed$state$hierarchy_layout
      )
    } else {
  ...
  if (!is.null(chart_type) && nrow(templates_chart_type_data) > 0) {
    ...
    is.null(chart_type) &&
      !is.null(template_id) &&
      !is.null(template_version_number)
  ) {
    ...
    stop(
      "Error: specify only a chart type or a template ID and version number."
    )
    ...
    stop(
      "Error: your chart type string is not an exact match with Flourish's available graphs."
    )
  } else if (grepl("(^map_.*$)|(^globe_.*)", chart_type)) {
    stop(
      "Error: we do not currently support visualizing map templates via the chart type parameter."
    )
  }
  ...
  if (isTRUE(knitr::is_html_output())) {
    iframe_src <- paste0(
      "https://flourish-api.com/api/v1/live/template?api_key=",
      api_key,
      "&template=",
      template_id,
      "&api_version=",
  ...
  x <- list(
    ...
    state = c(
      flourish_default_layout(chart_description),
      chart_type = base_viz_chart_type,
