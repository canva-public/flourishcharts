...
set_audio_details <- function(
  ...
  if (!is.null(audio_player_title_weight)) {
    if (!audio_player_title_weight %in% list_audio_player_title_weight) {
      ...
      if (
        ...
        if (
          length(named_list) > 1 &&
            audio_player_title_weight %in% names(named_list)
        ) {
  ...
  if (!is.null(audio_player_subtitle_weight)) {
    if (!audio_player_subtitle_weight %in% list_audio_player_subtitle_weight) {
      ...
      if (
        ...
        if (
          length(named_list) > 1 &&
            audio_player_subtitle_weight %in% names(named_list)
        ) {
          if (is.logical(audio_player_subtitle_weight)) {
            audio_player_subtitle_weight <- as.character(
              audio_player_subtitle_weight
            )
          }
          named_list_value <- unname(named_list[audio_player_subtitle_weight])[[
            1
          ]]
  ...
  if (!is.null(audio_player_time_weight)) {
    if (!audio_player_time_weight %in% list_audio_player_time_weight) {
      ...
      if (
        ...
        if (
          length(named_list) > 1 &&
            audio_player_time_weight %in% names(named_list)
        ) {
  ...
  if (!is.null(chart_filter_controls_control_type)) {
    if (
      ...
      stop(
        "`chart_filter_controls_control_type` must be one of: c('dropdown', 'buttons')"
      )
    } else {
      if (
        length(jsonlite::fromJSON(
          '{"dropdown": "dropdown", "buttons": "buttons"}'
        )) >
          1
      ) {
