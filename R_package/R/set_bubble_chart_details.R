...
set_bubble_chart_details <- function(
  ...
  if (!is.null(bubble_alignment)) {
    if (!bubble_alignment %in% list_bubble_alignment) {
      ...
      if (
        length(jsonlite::fromJSON('{"center": "center", "axis": "axis"}')) > 1
      ) {
  ...
  if (!is.null(label_font_weight)) {
    if (!label_font_weight %in% list_label_font_weight) {
      ...
      if (
        ...
        names(named_list) <- boolean_names
        if (
          length(named_list) > 1 && label_font_weight %in% names(named_list)
        ) {
  ...
  if (!is.null(label_font_case)) {
    if (!label_font_case %in% list_label_font_case) {
      ...
      if (
        length(jsonlite::fromJSON('{"": "", "uppercase": "uppercase"}')) > 1
      ) {
  ...
  if (length(selected_bubbles) > 1) {
    new_list$x$state$selected_bubbles <- paste(
      selected_bubbles,
      collapse = "\n"
    )
  } else {
  ...
  if (!is.null(axis_legend_title_mode)) {
    if (!axis_legend_title_mode %in% list_axis_legend_title_mode) {
      ...
      if (
        ...
        if (
          length(named_list) > 1 &&
            axis_legend_title_mode %in% names(named_list)
        ) {
  ...
  if (!is.null(chart_size_legend_show_legend)) {
    if (
      !chart_size_legend_show_legend %in% list_chart_size_legend_show_legend
    ) {
      ...
      if (length(jsonlite::fromJSON('{"true": true, "false": false}')) > 1) {
        ...
        if (
          length(named_list) > 1 &&
            chart_size_legend_show_legend %in% names(named_list)
        ) {
          if (is.logical(chart_size_legend_show_legend)) {
            chart_size_legend_show_legend <- as.character(
              chart_size_legend_show_legend
            )
          }
          named_list_value <- unname(named_list[
            chart_size_legend_show_legend
  ...
  if (!is.null(chart_size_legend_title_mode)) {
    if (!chart_size_legend_title_mode %in% list_chart_size_legend_title_mode) {
      ...
      if (
        ...
        if (
          length(named_list) > 1 &&
            chart_size_legend_title_mode %in% names(named_list)
        ) {
          if (is.logical(chart_size_legend_title_mode)) {
            chart_size_legend_title_mode <- as.character(
              chart_size_legend_title_mode
            )
          }
          named_list_value <- unname(named_list[chart_size_legend_title_mode])[[
            1
          ]]
