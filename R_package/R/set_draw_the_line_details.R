...
set_draw_the_line_details <- function(
    ...
    if (!line_curve %in% list_line_curve) {
      ...
      if (
        length(jsonlite::fromJSON(
          '{"linear": "linear", "step": "step", "curve": "curve"}'
        )) >
          1
      ) {
  ...
  if (!is.null(guess_lines_panel_position)) {
    if (!guess_lines_panel_position %in% list_guess_lines_panel_position) {
      stop(
        "`guess_lines_panel_position` must be one of: c('top', 'middle', 'bottom')"
      )
    } else {
      if (
        length(jsonlite::fromJSON(
        ...
        if (
          length(named_list) > 1 &&
            guess_lines_panel_position %in% names(named_list)
        ) {
          if (is.logical(guess_lines_panel_position)) {
            guess_lines_panel_position <- as.character(
              guess_lines_panel_position
            )
          }
          named_list_value <- unname(named_list[guess_lines_panel_position])[[
            1
          ]]
  ...
  if (!is.null(datetime_input_format)) {
    if (!datetime_input_format %in% list_datetime_input_format) {
      stop(
        "`datetime_input_format` must be one of: c('auto', '%Y-%m-%dT%H:%M:%S.%LZ', '%Y-%m-%d', '%m/%d/%Y', '%d/%m/%Y', '%d-%b-%y', '%m/%Y', '%m/%y', '%b %Y', '%b %y', '%B %d', '%d %b', '%Y', '%y', '%B', '%b', '%H:%M:%S', '%I:%M %p', '%H:%M')"
      )
    } else {
      if (
        length(jsonlite::fromJSON(
          '{"auto": "auto", "%Y-%m-%dT%H:%M:%S.%LZ": "%Y-%m-%dT%H:%M:%S.%LZ", "%Y-%m-%d": "%Y-%m-%d", "%m/%d/%Y": "%m/%d/%Y", "%d/%m/%Y": "%d/%m/%Y", "%d-%b-%y": "%d-%b-%y", "%m/%Y": "%m/%Y", "%m/%y": "%m/%y", "%b %Y": "%b %Y", "%b %y": "%b %y", "%B %d": "%B %d", "%d %b": "%d %b", "%Y": "%Y", "%y": "%y", "%B": "%B", "%b": "%b", "%H:%M:%S": "%H:%M:%S", "%I:%M %p": "%I:%M %p", "%H:%M": "%H:%M"}'
        )) >
          1
      ) {
        named_list <- jsonlite::fromJSON(
        ...
        names(named_list) <- boolean_names
        if (
          length(named_list) > 1 && datetime_input_format %in% names(named_list)
        ) {
  ...
  if (!is.null(date_format)) {
    if (!date_format %in% list_date_format) {
      stop(
        "`date_format` must be one of: c('auto', '%Y-%m-%dT%H:%M:%S.%LZ', '%Y-%m-%d', '%m/%d/%Y', '%d/%m/%Y', '%d-%b-%y', '%m/%Y', '%m/%y', '%b %Y', '%b %y', '%B %d', '%d %b', '%Y', '%y', '%B', '%b', '%A', '%a', '%H:%M:%S', '%I:%M %p', '%H:%M')"
      )
    } else {
      if (
        length(jsonlite::fromJSON(
          '{"auto": "auto", "%Y-%m-%dT%H:%M:%S.%LZ": "%Y-%m-%dT%H:%M:%S.%LZ", "%Y-%m-%d": "%Y-%m-%d", "%m/%d/%Y": "%m/%d/%Y", "%d/%m/%Y": "%d/%m/%Y", "%d-%b-%y": "%d-%b-%y", "%m/%Y": "%m/%Y", "%m/%y": "%m/%y", "%b %Y": "%b %Y", "%b %y": "%b %y", "%B %d": "%B %d", "%d %b": "%d %b", "%Y": "%Y", "\u2019%y": "\u2019%y", "%B": "%B", "%b": "%b", "%A": "%A", "%a": "%a", "%H:%M:%S": "%H:%M:%S", "%I:%M %p": "%I:%M %p", "%H:%M": "%H:%M"}'
        )) >
          1
      ) {
        named_list <- jsonlite::fromJSON(
