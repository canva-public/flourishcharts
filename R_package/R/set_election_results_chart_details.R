...
#' @name set_election_results_chart_details
#' @param bar_height Height. Flourish type hint: number, defaults to `NULL`
#' @param bar_background Background. Background color of part of chart thats not filled with data (ie. when filtering parties). Flourish type hint: color, defaults to `NULL`
#' @param show_party_name Show party name. Flourish type hint: boolean, defaults to `NULL`
#' @param label_font_size Text size. Flourish type hint: number, defaults to `NULL`
#' @param label_font_color Text color. Flourish type hint: color, defaults to `NULL`
#' @param use_custom_scale Scale type. Choices: `FALSE`, `TRUE`. On default, will use the sum of all data points as the max value. Enable this setting to use a custom value. Flourish type hint: boolean, defaults to `NULL`
#' @param custom_scale_max Max value for scale. This will set the maximum value of the scale. Eg. the total amount of seats to be won in an election. Flourish type hint: number, defaults to `NULL`
#' @param middle_line_position Position. Flourish type hint: number, defaults to `NULL`
#' @param middle_line_content Text. Flourish type hint: string, defaults to `NULL`
#' @param middle_line_text_color Color. Flourish type hint: color, defaults to `NULL`
#' @param middle_line_dashoffset Dash width and offset. Length of dashes and spaces seperated by a comma for example, 2,2. Measured in pixels. Flourish type hint: string, defaults to `NULL`
#' @param middle_line_width Thickness. Flourish type hint: number, defaults to `NULL`
#' @param middle_line_color Color. Flourish type hint: color, defaults to `NULL`
...
#' @param legend_position Position. Choices: below, above. Flourish type hint: string, defaults to `NULL`
#' @param hide_empty_legend_items Hide zero-valued entries. Flourish type hint: boolean, defaults to `NULL`
#' @param placeholder_text No data placeholder text. Placeholder text if there is no data. Flourish type hint: string, defaults to `NULL`
#' @param subtitle_text Text below results. Flourish type hint: string, defaults to `NULL`
#' @param value_change_abbreviation Value change abbreviation. A suffix used to describe the change since the previous election. Flourish type hint: string, defaults to `NULL`
...
#' @param chart_popup_popup_custom_main Popup custom main content. You can use various keywords to add informtion to custom popups. These include:   `{{PARTY}}` : name of the party  `{{CURRENT}}` : current value  `{{HISTORICAL}}` : historical value  `{{CHANGE}}` : change between the current and historical values  Advanced users can add HTML styling or add category colors to popup headers.  To hide parts of the popup when a column is empty using `{{#if column_name}}Content for non-empty column_name{{/if}}`. For example: `{{#if Capital name}}Capital:{{/if}} {{Capital name}}`. Here, the word "Capital:" will only show if the corresponding cell in the "Capital name" column is not empty. Flourish type hint: html, defaults to `NULL`
...
#' @param label_suffix Historical label suffix. Suffix used to compare current and historical results. For example, "%" when data measures vote shares, or "seats" when data measures seats won. Flourish type hint: string, defaults to `NULL`
...
set_election_results_chart_details <- function(
  .,
  bar_height = NULL,
  bar_background = NULL,
  ...
  details_error(., "election_results_chart")

  old_list <- .
  new_list <- list()

  if (isTRUE(old_list$x$template != "@flourish/election-results-chart")) {
    stop(
      "`set_election_results_chart_details()` is not supported for this chart type."
    )
  }
  new_list$x$state$bar_height <- bar_height
  new_list$x$state$bar_background <- bar_background
  new_list$x$state$show_party_name <- show_party_name
  new_list$x$state$label_font_size <- label_font_size
  new_list$x$state$label_font_color <- label_font_color
  list_use_custom_scale <- c(FALSE, TRUE, NULL)
  if (!is.null(use_custom_scale)) {
    if (!use_custom_scale %in% list_use_custom_scale) {
      stop("`use_custom_scale` must be one of: c(FALSE, TRUE)")
    } else {
      if (length(jsonlite::fromJSON('{"false": false, "true": true}')) > 1) {
        ...
        if (length(named_list) > 1 && use_custom_scale %in% names(named_list)) {
          if (is.logical(use_custom_scale)) {
            use_custom_scale <- as.character(use_custom_scale)
          }
          named_list_value <- unname(named_list[use_custom_scale])[[1]]
          new_list$x$state$use_custom_scale <- named_list_value
        }
      }
    }
  }
  new_list$x$state$custom_scale_max <- custom_scale_max
  new_list$x$state$middle_line_position <- middle_line_position
  new_list$x$state$middle_line_content <- middle_line_content
  new_list$x$state$middle_line_text_color <- middle_line_text_color
  new_list$x$state$middle_line_dashoffset <- middle_line_dashoffset
  new_list$x$state$middle_line_width <- middle_line_width
  new_list$x$state$middle_line_color <- middle_line_color
  ...
  list_legend_position <- c("below", "above", NULL)
  if (!is.null(legend_position)) {
    if (!legend_position %in% list_legend_position) {
      stop("`legend_position` must be one of: c('below', 'above')")
    } else {
      if (
        length(jsonlite::fromJSON('{"below": "below", "above": "above"}')) > 1
      ) {
        named_list <- jsonlite::fromJSON('{"below": "below", "above": "above"}')
        # Get the names of the list
        names_list <- names(named_list)
        # Convert names to boolean values only if they are "true" or "false"
        boolean_names <- sapply(names_list, function(x) {
  ...
  new_list$x$state$hide_empty_legend_items <- hide_empty_legend_items
  new_list$x$state$placeholder_text <- placeholder_text
  new_list$x$state$subtitle_text <- subtitle_text
  new_list$x$state$value_change_abbreviation <- value_change_abbreviation
  ...
  new_list$x$state$label_suffix <- label_suffix
