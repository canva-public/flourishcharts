...
set_gantt_details <- function(
  ...
  if (!is.null(gantt_font_weight)) {
    if (!gantt_font_weight %in% list_gantt_font_weight) {
      ...
      if (
        ...
        names(named_list) <- boolean_names
        if (
          length(named_list) > 1 && gantt_font_weight %in% names(named_list)
        ) {
  ...
  if (!is.null(chart_background_panels)) {
    if (!chart_background_panels %in% list_chart_background_panels) {
      ...
      if (length(jsonlite::fromJSON('{"false": false, "true": true}')) > 1) {
        ...
        if (
          length(named_list) > 1 &&
            chart_background_panels %in% names(named_list)
        ) {
  ...
  if (!is.null(date_format)) {
    if (!date_format %in% list_date_format) {
      stop(
        "`date_format` must be one of: c('auto', '%Y-%m-%dT%H:%M:%S.%LZ', '%Y-%m-%d', '%m/%d/%Y', '%d/%m/%Y', '%d-%b-%y', '%m/%Y', '%b %Y', '%b %y', '%B %d', '%d %b', '%Y', '%B', '%b', '%A', '%a', '%H:%M:%S', '%I:%M %p', '%H:%M')"
      )
    } else {
      if (
        length(jsonlite::fromJSON(
          '{"auto": "auto", "%Y-%m-%dT%H:%M:%S.%LZ": "%Y-%m-%dT%H:%M:%S.%LZ", "%Y-%m-%d": "%Y-%m-%d", "%m/%d/%Y": "%m/%d/%Y", "%d/%m/%Y": "%d/%m/%Y", "%d-%b-%y": "%d-%b-%y", "%m/%Y": "%m/%Y", "%b %Y": "%b %Y", "%b %y": "%b %y", "%B %d": "%B %d", "%d %b": "%d %b", "%Y": "%Y", "%B": "%B", "%b": "%b", "%A": "%A", "%a": "%a", "%H:%M:%S": "%H:%M:%S", "%I:%M %p": "%I:%M %p", "%H:%M": "%H:%M"}'
        )) >
          1
      ) {
        named_list <- jsonlite::fromJSON(
  ...
  if (!is.null(format_weekday)) {
    if (!format_weekday %in% list_format_weekday) {
      stop(
        "`format_weekday` must be one of: c('none', 'narrow', 'short', 'long')"
      )
    } else {
      if (
        length(jsonlite::fromJSON(
          '{"none": "none", "narrow": "narrow", "short": "short", "long": "long"}'
        )) >
          1
      ) {
        named_list <- jsonlite::fromJSON(
  ...
  if (!is.null(format_year)) {
    if (!format_year %in% list_format_year) {
      ...
      if (
        length(jsonlite::fromJSON(
          '{"none": "none", "numeric": "numeric", "2-digit": "2-digit"}'
        )) >
          1
      ) {
        named_list <- jsonlite::fromJSON(
  ...
  if (!is.null(format_month)) {
    if (!format_month %in% list_format_month) {
      stop(
        "`format_month` must be one of: c('none', 'numeric', '2-digit', 'narrow', 'short', 'long')"
      )
    } else {
      if (
        length(jsonlite::fromJSON(
          '{"none": "none", "numeric": "numeric", "2-digit": "2-digit", "narrow": "narrow", "short": "short", "long": "long"}'
        )) >
          1
      ) {
        named_list <- jsonlite::fromJSON(
