...
set_number_ticker_details <- function(
  ...
  if (!is.null(mode)) {
    if (!mode %in% list_mode) {
      ...
      if (
        length(jsonlite::fromJSON('{"simple": "simple", "custom": "custom"}')) >
          1
      ) {
        named_list <- jsonlite::fromJSON(
          '{"simple": "simple", "custom": "custom"}'
        )
  ...
  if (!is.null(easing)) {
    if (!easing %in% list_easing) {
      stop(
        "`easing` must be one of: c('constant', 'speed_up', 'slow_down', 'fast_slow_fast')"
      )
    } else {
      if (
        length(jsonlite::fromJSON(
          '{"constant": "constant", "speed_up": "speed_up", "slow_down": "slow_down", "fast_slow_fast": "fast_slow_fast"}'
        )) >
          1
      ) {
        named_list <- jsonlite::fromJSON(
  ...
  list_value_format_negative_sign <- c(
    "-$nk",
    "$-nk",
    "($nk)",
    "$(n)k",
  ...
  if (!is.null(value_format_negative_sign)) {
    if (!value_format_negative_sign %in% list_value_format_negative_sign) {
      stop(
        "`value_format_negative_sign` must be one of: c('-$nk', '$-nk', '($nk)', '$(n)k', 'none')"
      )
    } else {
      if (
        length(jsonlite::fromJSON(
        ...
        if (
          length(named_list) > 1 &&
            value_format_negative_sign %in% names(named_list)
        ) {
          if (is.logical(value_format_negative_sign)) {
            value_format_negative_sign <- as.character(
              value_format_negative_sign
            )
          }
          named_list_value <- unname(named_list[value_format_negative_sign])[[
            1
          ]]
  ...
  if (!is.null(value_format_transform)) {
    if (!value_format_transform %in% list_value_format_transform) {
      stop(
        "`value_format_transform` must be one of: c('multiply', 'divide', 'exponentiate')"
      )
    } else {
      if (
        length(jsonlite::fromJSON(
        ...
        if (
          length(named_list) > 1 &&
            value_format_transform %in% names(named_list)
        ) {
