...
set_quiz_details <- function(
  ...
  if (!is.null(block_btn_text_size)) {
    if (!block_btn_text_size %in% list_block_btn_text_size) {
      stop(
        "`block_btn_text_size` must be one of: c('1.1', '1.2', '1.4', 'custom')"
      )
    } else {
      if (
        length(jsonlite::fromJSON(
          '{"1.1": "1.1", "1.2": "1.2", "1.4": "1.4", "custom": "custom"}'
        )) >
          1
      ) {
        named_list <- jsonlite::fromJSON(
        ...
        names(named_list) <- boolean_names
        if (
          length(named_list) > 1 && block_btn_text_size %in% names(named_list)
        ) {
  ...
  if (!is.null(slider_range_type)) {
    if (!slider_range_type %in% list_slider_range_type) {
      ...
      if (
        ...
        names(named_list) <- boolean_names
        if (
          length(named_list) > 1 && slider_range_type %in% names(named_list)
        ) {
  ...
  if (length(slider_range_custom) > 1) {
    new_list$x$state$slider_range_custom <- paste(
      slider_range_custom,
      collapse = "\n"
    )
  } else {
  ...
  if (length(end_result_default_text) > 1) {
    new_list$x$state$end_result_default_text <- paste(
