...
set_slope_chart_details <- function(
    ...
    if (!curve %in% list_curve) {
      ...
      if (
        length(jsonlite::fromJSON(
          '{"linear": "linear", "cardinal": "cardinal", "step": "step"}'
        )) >
          1
      ) {
  ...
  if (!is.null(value_type)) {
    if (!value_type %in% list_value_type) {
      ...
      if (
        length(jsonlite::fromJSON(
          '{"ranks": "ranks", "scores": "scores", "change": "change"}'
        )) >
          1
      ) {
        named_list <- jsonlite::fromJSON(
  ...
  if (!is.null(ties_mode)) {
    if (!ties_mode %in% list_ties_mode) {
      stop(
        "`ties_mode` must be one of: c('dense', 'competition', 'fractional', 'modified_competition')"
      )
    } else {
      if (
        length(jsonlite::fromJSON(
          '{"dense": "dense", "competition": "competition", "fractional": "fractional", "modified_competition": "modified_competition"}'
        )) >
          1
      ) {
        named_list <- jsonlite::fromJSON(
  ...
  if (!is.null(labels_name_position)) {
    if (!labels_name_position %in% list_labels_name_position) {
      ...
      if (
        length(jsonlite::fromJSON(
          '{"side": "side", "inline": "inline", "none": "none"}'
        )) >
          1
      ) {
        named_list <- jsonlite::fromJSON(
        ...
        names(named_list) <- boolean_names
        if (
          length(named_list) > 1 && labels_name_position %in% names(named_list)
        ) {
  ...
  if (!is.null(labels_inline_position)) {
    if (!labels_inline_position %in% list_labels_inline_position) {
      stop(
        "`labels_inline_position` must be one of: c('start', 'middle', 'end')"
      )
    } else {
      if (
        length(jsonlite::fromJSON(
        ...
        if (
          length(named_list) > 1 &&
            labels_inline_position %in% names(named_list)
        ) {
  ...
  if (length(labels_inline_position_custom) > 1) {
    new_list$x$state$labels_inline_position_custom <- paste(
  ...
  if (!is.null(axis_labels_position)) {
    if (!axis_labels_position %in% list_axis_labels_position) {
      stop(
        "`axis_labels_position` must be one of: c('top', 'bottom', 'hidden')"
      )
    } else {
      if (
        length(jsonlite::fromJSON(
        ...
        names(named_list) <- boolean_names
        if (
          length(named_list) > 1 && axis_labels_position %in% names(named_list)
        ) {
