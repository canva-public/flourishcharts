...
set_sports_details <- function(
    ...
    if (!positions_type %in% list_positions_type) {
      ...
      if (
        length(jsonlite::fromJSON(
          '{"auto": "auto", "formation": "formation", "data": "data"}'
        )) >
          1
      ) {
  ...
  if (!is.null(positions_scale_custom)) {
    if (!positions_scale_custom %in% list_positions_scale_custom) {
      ...
      if (
        ...
        if (
          length(named_list) > 1 &&
            positions_scale_custom %in% names(named_list)
        ) {
  ...
  if (!is.null(formation_type)) {
    if (!formation_type %in% list_formation_type) {
      ...
      if (
        length(jsonlite::fromJSON('{"preset": "preset", "custom": "custom"}')) >
          1
      ) {
        named_list <- jsonlite::fromJSON(
  ...
  if (!is.null(formation)) {
    if (!formation %in% list_formation) {
      stop(
        "`formation` must be one of: c('3-5-2', '4-3-3', '4-3-2-1', '4-4-2', '3-4-3', '4-2-3-1')"
      )
    } else {
      if (
        length(jsonlite::fromJSON(
  ...
  if (length(formation_custom) > 1) {
    new_list$x$state$formation_custom <- paste(
      formation_custom,
      collapse = "\n"
    )
  } else {
  ...
  if (!is.null(pitch_type)) {
    if (!pitch_type %in% list_pitch_type) {
      stop(
        "`pitch_type` must be one of: c('football', 'basketball', 'tennis', 'custom')"
      )
    } else {
      if (
        length(jsonlite::fromJSON(
          '{"football": "football", "basketball": "basketball", "tennis": "tennis", "custom": "custom"}'
        )) >
          1
      ) {
        named_list <- jsonlite::fromJSON(
  ...
  if (!is.null(pitch_type_custom)) {
    if (!pitch_type_custom %in% list_pitch_type_custom) {
      ...
      if (
        length(jsonlite::fromJSON('{"upload": "upload", "svg": "svg"}')) > 1
      ) {
        ...
        names(named_list) <- boolean_names
        if (
          length(named_list) > 1 && pitch_type_custom %in% names(named_list)
        ) {
  ...
  if (!is.null(pitch_orientation)) {
    if (!pitch_orientation %in% list_pitch_orientation) {
      ...
      if (
        ...
        names(named_list) <- boolean_names
        if (
          length(named_list) > 1 && pitch_orientation %in% names(named_list)
        ) {
  ...
  if (length(colors_categorical_palette) > 1) {
    new_list$x$state$colors$categorical_palette <- paste(
      colors_categorical_palette,
      collapse = "\n"
    )
  } else {
  ...
  if (length(colors_categorical_custom_palette) > 1) {
    new_list$x$state$colors$categorical_custom_palette <- paste(
      colors_categorical_custom_palette,
      collapse = "\n"
    )
  } else {
  ...
  if (!is.null(colors_numeric_type)) {
    if (!colors_numeric_type %in% list_colors_numeric_type) {
      ...
      if (
        ...
        names(named_list) <- boolean_names
        if (
          length(named_list) > 1 && colors_numeric_type %in% names(named_list)
        ) {
  ...
  if (!is.null(colors_sequential_color_space)) {
    if (
      !colors_sequential_color_space %in% list_colors_sequential_color_space
    ) {
      stop(
        "`colors_sequential_color_space` must be one of: c('rgb', 'lab', 'hcl', 'hsl')"
      )
    } else {
      if (
        length(jsonlite::fromJSON(
        ...
        if (
          length(named_list) > 1 &&
            colors_sequential_color_space %in% names(named_list)
        ) {
          if (is.logical(colors_sequential_color_space)) {
            colors_sequential_color_space <- as.character(
              colors_sequential_color_space
            )
          }
  ...
  if (!is.null(colors_sequential_custom_domain)) {
    if (
      !colors_sequential_custom_domain %in% list_colors_sequential_custom_domain
    ) {
      ...
      if (length(jsonlite::fromJSON('{"false": false, "true": true}')) > 1) {
        ...
        if (
          length(named_list) > 1 &&
            colors_sequential_custom_domain %in% names(named_list)
        ) {
          if (is.logical(colors_sequential_custom_domain)) {
            colors_sequential_custom_domain <- as.character(
              colors_sequential_custom_domain
            )
          }
          named_list_value <- unname(named_list[
            colors_sequential_custom_domain
  ...
  if (!is.null(colors_diverging_color_space)) {
    if (!colors_diverging_color_space %in% list_colors_diverging_color_space) {
      stop(
        "`colors_diverging_color_space` must be one of: c('rgb', 'lab', 'hcl', 'hsl')"
      )
    } else {
      if (
        length(jsonlite::fromJSON(
        ...
        if (
          length(named_list) > 1 &&
            colors_diverging_color_space %in% names(named_list)
        ) {
          if (is.logical(colors_diverging_color_space)) {
            colors_diverging_color_space <- as.character(
              colors_diverging_color_space
            )
          }
          named_list_value <- unname(named_list[colors_diverging_color_space])[[
            1
          ]]
  ...
  if (!is.null(colors_diverging_custom_domain)) {
    if (
      !colors_diverging_custom_domain %in% list_colors_diverging_custom_domain
    ) {
      ...
      if (length(jsonlite::fromJSON('{"false": false, "true": true}')) > 1) {
        ...
        if (
          length(named_list) > 1 &&
            colors_diverging_custom_domain %in% names(named_list)
        ) {
          if (is.logical(colors_diverging_custom_domain)) {
            colors_diverging_custom_domain <- as.character(
  ...
  if (!is.null(dot_size)) {
    if (!dot_size %in% list_dot_size) {
      ...
      if (
        length(jsonlite::fromJSON(
          '{"1.3": "1.3", "2.5": "2.5", "6": "6", "custom": "custom"}'
        )) >
          1
      ) {
        named_list <- jsonlite::fromJSON(
  ...
  if (!is.null(dot_border)) {
    if (!dot_border %in% list_dot_border) {
      ...
      if (
        length(jsonlite::fromJSON(
          '{"0": "0", "0.2": "0.2", "0.5": "0.5", "custom": "custom"}'
        )) >
          1
      ) {
        named_list <- jsonlite::fromJSON(
  ...
  if (!is.null(label_size)) {
    if (!label_size %in% list_label_size) {
      ...
      if (
        length(jsonlite::fromJSON(
          '{"0.8": "0.8", "1": "1", "1.2": "1.2", "custom": "custom"}'
        )) >
          1
      ) {
        named_list <- jsonlite::fromJSON(
  ...
  if (!is.null(squad_number_size)) {
    if (!squad_number_size %in% list_squad_number_size) {
      stop(
        "`squad_number_size` must be one of: c('1.1', '1.4', '1.7', 'custom')"
      )
    } else {
      if (
        length(jsonlite::fromJSON(
          '{"1.1": "1.1", "1.4": "1.4", "1.7": "1.7", "custom": "custom"}'
        )) >
          1
      ) {
        named_list <- jsonlite::fromJSON(
        ...
        names(named_list) <- boolean_names
        if (
          length(named_list) > 1 && squad_number_size %in% names(named_list)
        ) {
  ...
  if (!is.null(squad_number_weight)) {
    if (!squad_number_weight %in% list_squad_number_weight) {
      ...
      if (
        ...
        names(named_list) <- boolean_names
        if (
          length(named_list) > 1 && squad_number_weight %in% names(named_list)
        ) {
  ...
  if (!is.null(image_blend_mode)) {
    if (!image_blend_mode %in% list_image_blend_mode) {
      stop(
        "`image_blend_mode` must be one of: c('normal', 'multiply', 'screen', 'overlay', 'darken', 'lighten')"
      )
    } else {
      if (
        length(jsonlite::fromJSON(
  ...
  if (!is.null(animation_target)) {
    if (!animation_target %in% list_animation_target) {
      stop(
        "`animation_target` must be one of: c('position', 'opacity', 'both')"
      )
    } else {
      if (
        length(jsonlite::fromJSON(
          '{"position": "position", "opacity": "opacity", "both": "both"}'
        )) >
          1
      ) {
        named_list <- jsonlite::fromJSON(
  ...
  if (!is.null(trail_style)) {
    if (!trail_style %in% list_trail_style) {
      ...
      if (
        length(jsonlite::fromJSON('{"solid": "solid", "dotted": "dotted"}')) > 1
      ) {
        named_list <- jsonlite::fromJSON(
