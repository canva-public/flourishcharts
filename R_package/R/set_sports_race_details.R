...
set_sports_race_details <- function(
  ...
  if (!is.null(preset_svg)) {
    if (!preset_svg %in% list_preset_svg) {
      stop(
        "`preset_svg` must be one of: c('athletics', 'swimming', 'cycling', 'custom')"
      )
    } else {
      if (
        length(jsonlite::fromJSON(
          '{"athletics": "athletics", "swimming": "swimming", "cycling": "cycling", "custom": "custom"}'
        )) >
          1
      ) {
  ...
  if (!is.null(track_back_and_forth)) {
    if (!track_back_and_forth %in% list_track_back_and_forth) {
      ...
      if (length(jsonlite::fromJSON('{"false": false, "true": true}')) > 1) {
        ...
        names(named_list) <- boolean_names
        if (
          length(named_list) > 1 && track_back_and_forth %in% names(named_list)
        ) {
  ...
  if (!is.null(participant_icon_mode)) {
    if (!participant_icon_mode %in% list_participant_icon_mode) {
      ...
      if (
        length(jsonlite::fromJSON('{"circle": "circle", "image": "image"}')) > 1
      ) {
        named_list <- jsonlite::fromJSON(
          '{"circle": "circle", "image": "image"}'
        )
        # Get the names of the list
        names_list <- names(named_list)
        # Convert names to boolean values only if they are "true" or "false"
        ...
        names(named_list) <- boolean_names
        if (
          length(named_list) > 1 && participant_icon_mode %in% names(named_list)
        ) {
  ...
  list_preset_participant_image <- c(
    "runner",
    "swimmer",
    "cyclist",
    "custom",
  ...
  if (!is.null(preset_participant_image)) {
    if (!preset_participant_image %in% list_preset_participant_image) {
      stop(
        "`preset_participant_image` must be one of: c('runner', 'swimmer', 'cyclist', 'custom')"
      )
    } else {
      if (
        length(jsonlite::fromJSON(
          '{"runner": "runner", "swimmer": "swimmer", "cyclist": "cyclist", "custom": "custom"}'
        )) >
          1
      ) {
        named_list <- jsonlite::fromJSON(
        ...
        if (
          length(named_list) > 1 &&
            preset_participant_image %in% names(named_list)
        ) {
  ...
  if (!is.null(image_initial_rotation)) {
    if (!image_initial_rotation %in% list_image_initial_rotation) {
      stop(
        "`image_initial_rotation` must be one of: c('0', '90', '180', '270')"
      )
    } else {
      if (
        length(jsonlite::fromJSON(
        ...
        if (
          length(named_list) > 1 &&
            image_initial_rotation %in% names(named_list)
        ) {
  ...
  if (!is.null(image_transform_mode)) {
    if (!image_transform_mode %in% list_image_transform_mode) {
      stop(
        "`image_transform_mode` must be one of: c('rotate', 'flip_x', 'flip_y', 'flip_both', 'static')"
      )
    } else {
      if (
        length(jsonlite::fromJSON(
          '{"rotate": "rotate", "flip_x": "flip_x", "flip_y": "flip_y", "flip_both": "flip_both", "static": "static"}'
        )) >
          1
      ) {
        named_list <- jsonlite::fromJSON(
        ...
        names(named_list) <- boolean_names
        if (
          length(named_list) > 1 && image_transform_mode %in% names(named_list)
        ) {
  ...
  if (length(chart_participant_color_scale_categorical_palette) > 1) {
    new_list$x$state$participant_color$categorical_palette <- paste(
  ...
  if (length(chart_participant_color_scale_categorical_custom_palette) > 1) {
    new_list$x$state$participant_color$categorical_custom_palette <- paste(
  ...
  list_chart_participant_color_scale_numeric_type <- c(
    "sequential",
    "diverging",
    NULL
  )
  if (!is.null(chart_participant_color_scale_numeric_type)) {
    if (
      !chart_participant_color_scale_numeric_type %in%
        list_chart_participant_color_scale_numeric_type
    ) {
      stop(
        "`chart_participant_color_scale_numeric_type` must be one of: c('sequential', 'diverging')"
      )
    } else {
      if (
        length(jsonlite::fromJSON(
        ...
        if (
          length(named_list) > 1 &&
            chart_participant_color_scale_numeric_type %in% names(named_list)
        ) {
          if (is.logical(chart_participant_color_scale_numeric_type)) {
            chart_participant_color_scale_numeric_type <- as.character(
  ...
  if (!is.null(chart_participant_color_scale_binning)) {
    if (
      !chart_participant_color_scale_binning %in%
        list_chart_participant_color_scale_binning
    ) {
      stop(
        "`chart_participant_color_scale_binning` must be one of: c(FALSE, TRUE)"
      )
    } else {
      if (length(jsonlite::fromJSON('{"false": false, "true": true}')) > 1) {
        named_list <- jsonlite::fromJSON('{"false": false, "true": true}')
        ...
        if (
          length(named_list) > 1 &&
            chart_participant_color_scale_binning %in% names(named_list)
        ) {
          if (is.logical(chart_participant_color_scale_binning)) {
            chart_participant_color_scale_binning <- as.character(
  ...
  list_chart_participant_color_scale_bin_mode <- c(
    "fixed",
    "quantile",
    "custom",
    NULL
  ...
  if (!is.null(chart_participant_color_scale_bin_mode)) {
    if (
      !chart_participant_color_scale_bin_mode %in%
        list_chart_participant_color_scale_bin_mode
    ) {
      stop(
        "`chart_participant_color_scale_bin_mode` must be one of: c('fixed', 'quantile', 'custom')"
      )
    } else {
      if (
        length(jsonlite::fromJSON(
        ...
        if (
          length(named_list) > 1 &&
            chart_participant_color_scale_bin_mode %in% names(named_list)
        ) {
          if (is.logical(chart_participant_color_scale_bin_mode)) {
            chart_participant_color_scale_bin_mode <- as.character(
  ...
  list_chart_participant_color_scale_sequential_color_space <- c(
    "rgb",
    "lab",
    "hcl",
    "hsl",
  ...
  if (!is.null(chart_participant_color_scale_sequential_color_space)) {
    if (
      !chart_participant_color_scale_sequential_color_space %in%
        list_chart_participant_color_scale_sequential_color_space
    ) {
      stop(
        "`chart_participant_color_scale_sequential_color_space` must be one of: c('rgb', 'lab', 'hcl', 'hsl')"
      )
    } else {
      if (
        length(jsonlite::fromJSON(
        ...
        if (
          ...
          if (
            is.logical(chart_participant_color_scale_sequential_color_space)
          ) {
            chart_participant_color_scale_sequential_color_space <- as.character(
  ...
  list_chart_participant_color_scale_sequential_custom_domain <- c(
    FALSE,
    TRUE,
    NULL
  )
  if (!is.null(chart_participant_color_scale_sequential_custom_domain)) {
    if (
      !chart_participant_color_scale_sequential_custom_domain %in%
        list_chart_participant_color_scale_sequential_custom_domain
    ) {
      stop(
        "`chart_participant_color_scale_sequential_custom_domain` must be one of: c(FALSE, TRUE)"
      )
    } else {
      if (length(jsonlite::fromJSON('{"false": false, "true": true}')) > 1) {
        named_list <- jsonlite::fromJSON('{"false": false, "true": true}')
        ...
        if (
          ...
          if (
            is.logical(chart_participant_color_scale_sequential_custom_domain)
          ) {
            chart_participant_color_scale_sequential_custom_domain <- as.character(
  ...
  list_chart_participant_color_scale_diverging_color_space <- c(
    "rgb",
    "lab",
    "hcl",
    "hsl",
  ...
  if (!is.null(chart_participant_color_scale_diverging_color_space)) {
    if (
      !chart_participant_color_scale_diverging_color_space %in%
        list_chart_participant_color_scale_diverging_color_space
    ) {
      stop(
        "`chart_participant_color_scale_diverging_color_space` must be one of: c('rgb', 'lab', 'hcl', 'hsl')"
      )
    } else {
      if (
        length(jsonlite::fromJSON(
        ...
        if (
          ...
          if (is.logical(chart_participant_color_scale_diverging_color_space)) {
            chart_participant_color_scale_diverging_color_space <- as.character(
  ...
  list_chart_participant_color_scale_diverging_custom_domain <- c(
    FALSE,
    TRUE,
    NULL
  )
  if (!is.null(chart_participant_color_scale_diverging_custom_domain)) {
    if (
      !chart_participant_color_scale_diverging_custom_domain %in%
        list_chart_participant_color_scale_diverging_custom_domain
    ) {
      stop(
        "`chart_participant_color_scale_diverging_custom_domain` must be one of: c(FALSE, TRUE)"
      )
    } else {
      if (length(jsonlite::fromJSON('{"false": false, "true": true}')) > 1) {
        named_list <- jsonlite::fromJSON('{"false": false, "true": true}')
        ...
        if (
          ...
          if (
            is.logical(chart_participant_color_scale_diverging_custom_domain)
          ) {
            chart_participant_color_scale_diverging_custom_domain <- as.character(
  ...
  if (!is.null(camera_mode)) {
    if (!camera_mode %in% list_camera_mode) {
      ...
      if (
        length(jsonlite::fromJSON('{"track": "track", "leader": "leader"}')) > 1
      ) {
        named_list <- jsonlite::fromJSON(
          '{"track": "track", "leader": "leader"}'
        )
        # Get the names of the list
        names_list <- names(named_list)
        # Convert names to boolean values only if they are "true" or "false"
  ...
  if (!is.null(checkpoint_mode)) {
    if (!checkpoint_mode %in% list_checkpoint_mode) {
      ...
      if (
        length(jsonlite::fromJSON(
          '{"laps": "laps", "splits": "splits", "custom": "custom"}'
        )) >
          1
      ) {
        named_list <- jsonlite::fromJSON(
  ...
  if (!is.null(participant_label_mode)) {
    if (!participant_label_mode %in% list_participant_label_mode) {
      stop(
        "`participant_label_mode` must be one of: c('auto', 'leader', 'all', 'custom')"
      )
    } else {
      if (
        length(jsonlite::fromJSON(
          '{"auto": "auto", "leader": "leader", "all": "all", "custom": "custom"}'
        )) >
          1
      ) {
        named_list <- jsonlite::fromJSON(
        ...
        if (
          length(named_list) > 1 &&
            participant_label_mode %in% names(named_list)
        ) {
  ...
  if (length(participant_label_custom) > 1) {
    new_list$x$state$participant_label_custom <- paste(
  ...
  if (!is.null(participant_label_position)) {
    if (!participant_label_position %in% list_participant_label_position) {
      stop(
        "`participant_label_position` must be one of: c('auto', 'left', 'right')"
      )
    } else {
      if (
        length(jsonlite::fromJSON(
        ...
        if (
          length(named_list) > 1 &&
            participant_label_position %in% names(named_list)
        ) {
          if (is.logical(participant_label_position)) {
            participant_label_position <- as.character(
              participant_label_position
            )
          }
          named_list_value <- unname(named_list[participant_label_position])[[
            1
          ]]
  ...
  if (!is.null(participant_label_weight)) {
    if (!participant_label_weight %in% list_participant_label_weight) {
      ...
      if (length(jsonlite::fromJSON('{"400": "400", "600": "600"}')) > 1) {
        ...
        if (
          length(named_list) > 1 &&
            participant_label_weight %in% names(named_list)
        ) {
  ...
  if (!is.null(label_position_enabled)) {
    if (!label_position_enabled %in% list_label_position_enabled) {
      ...
      if (length(jsonlite::fromJSON('{"false": false, "true": true}')) > 1) {
        ...
        if (
          length(named_list) > 1 &&
            label_position_enabled %in% names(named_list)
        ) {
  ...
  if (!is.null(label_position_time)) {
    if (!label_position_time %in% list_label_position_time) {
      stop(
        "`label_position_time` must be one of: c('always', 'race', 'finish')"
      )
    } else {
      if (
        length(jsonlite::fromJSON(
          '{"always": "always", "race": "race", "finish": "finish"}'
        )) >
          1
      ) {
        named_list <- jsonlite::fromJSON(
        ...
        names(named_list) <- boolean_names
        if (
          length(named_list) > 1 && label_position_time %in% names(named_list)
        ) {
  ...
  if (!is.null(label_position_type)) {
    if (!label_position_type %in% list_label_position_type) {
      ...
      if (
        ...
        names(named_list) <- boolean_names
        if (
          length(named_list) > 1 && label_position_type %in% names(named_list)
        ) {
  ...
  if (!is.null(label_medal_enabled)) {
    if (!label_medal_enabled %in% list_label_medal_enabled) {
      ...
      if (length(jsonlite::fromJSON('{"false": false, "true": true}')) > 1) {
        ...
        names(named_list) <- boolean_names
        if (
          length(named_list) > 1 && label_medal_enabled %in% names(named_list)
        ) {
  ...
  if (!is.null(label_finishing_time_enabled)) {
    if (!label_finishing_time_enabled %in% list_label_finishing_time_enabled) {
      ...
      if (length(jsonlite::fromJSON('{"false": false, "true": true}')) > 1) {
        ...
        if (
          length(named_list) > 1 &&
            label_finishing_time_enabled %in% names(named_list)
        ) {
          if (is.logical(label_finishing_time_enabled)) {
            label_finishing_time_enabled <- as.character(
              label_finishing_time_enabled
            )
          }
          named_list_value <- unname(named_list[label_finishing_time_enabled])[[
            1
          ]]
  ...
  if (!is.null(label_finishing_time_time)) {
    if (!label_finishing_time_time %in% list_label_finishing_time_time) {
      ...
      if (
        length(jsonlite::fromJSON('{"always": "always", "finish": "finish"}')) >
          1
      ) {
        named_list <- jsonlite::fromJSON(
          '{"always": "always", "finish": "finish"}'
        )
        # Get the names of the list
        names_list <- names(named_list)
        # Convert names to boolean values only if they are "true" or "false"
        ...
        if (
          length(named_list) > 1 &&
            label_finishing_time_time %in% names(named_list)
        ) {
  ...
  if (!is.null(participant_stacking)) {
    if (!participant_stacking %in% list_participant_stacking) {
      stop(
        "`participant_stacking` must be one of: c('sheet_order', 'rank', 'y_position')"
      )
    } else {
      if (
        length(jsonlite::fromJSON(
          '{"sheet_order": "sheet_order", "rank": "rank", "y_position": "y_position"}'
        )) >
          1
      ) {
        named_list <- jsonlite::fromJSON(
        ...
        names(named_list) <- boolean_names
        if (
          length(named_list) > 1 && participant_stacking %in% names(named_list)
        ) {
  ...
  if (!is.null(chart_timeline_controls_enabled)) {
    if (
      !chart_timeline_controls_enabled %in% list_chart_timeline_controls_enabled
    ) {
      ...
      if (length(jsonlite::fromJSON('{"true": true, "false": false}')) > 1) {
        ...
        if (
          length(named_list) > 1 &&
            chart_timeline_controls_enabled %in% names(named_list)
        ) {
          if (is.logical(chart_timeline_controls_enabled)) {
            chart_timeline_controls_enabled <- as.character(
  ...
  if (!is.null(chart_timeline_controls_style)) {
    if (
      !chart_timeline_controls_style %in% list_chart_timeline_controls_style
    ) {
      stop(
        "`chart_timeline_controls_style` must be one of: c('timeline', 'button')"
      )
    } else {
      if (
        length(jsonlite::fromJSON(
          '{"timeline": "timeline", "button": "button"}'
        )) >
          1
      ) {
        named_list <- jsonlite::fromJSON(
        ...
        if (
          length(named_list) > 1 &&
            chart_timeline_controls_style %in% names(named_list)
        ) {
          if (is.logical(chart_timeline_controls_style)) {
            chart_timeline_controls_style <- as.character(
  ...
  if (!is.null(chart_timeline_controls_graph)) {
    if (
      !chart_timeline_controls_graph %in% list_chart_timeline_controls_graph
    ) {
      ...
      if (length(jsonlite::fromJSON('{"true": true, "false": false}')) > 1) {
        ...
        if (
          length(named_list) > 1 &&
            chart_timeline_controls_graph %in% names(named_list)
        ) {
          if (is.logical(chart_timeline_controls_graph)) {
            chart_timeline_controls_graph <- as.character(
  ...
  list_chart_timeline_controls_date_format_display <- c(
    "",
    "%Y-%m-%dT%H:%M:%S.%LZ",
    "%Y-%m-%d",
    "%m/%d/%Y",
  ...
  if (!is.null(chart_timeline_controls_date_format_display)) {
    if (
      !chart_timeline_controls_date_format_display %in%
        list_chart_timeline_controls_date_format_display
    ) {
      stop(
        "`chart_timeline_controls_date_format_display` must be one of: c('', '%Y-%m-%dT%H:%M:%S.%LZ', '%Y-%m-%d', '%m/%d/%Y', '%d/%m/%Y', '%d-%b-%y', '%m/%Y', '%b %Y', '%b %y', '%B %d', '%d %b', '%Y', '%B', '%b', '%A', '%a', '%H:%M:%S', '%I:%M %p', '%H:%M')"
      )
    } else {
      if (
        length(jsonlite::fromJSON(
          '{"": "", "%Y-%m-%dT%H:%M:%S.%LZ": "%Y-%m-%dT%H:%M:%S.%LZ", "%Y-%m-%d": "%Y-%m-%d", "%m/%d/%Y": "%m/%d/%Y", "%d/%m/%Y": "%d/%m/%Y", "%d-%b-%y": "%d-%b-%y", "%m/%Y": "%m/%Y", "%b %Y": "%b %Y", "%b %y": "%b %y", "%B %d": "%B %d", "%d %b": "%d %b", "%Y": "%Y", "%B": "%B", "%b": "%b", "%A": "%A", "%a": "%a", "%H:%M:%S": "%H:%M:%S", "%I:%M %p": "%I:%M %p", "%H:%M": "%H:%M"}'
        )) >
          1
      ) {
        named_list <- jsonlite::fromJSON(
        ...
        if (
          length(named_list) > 1 &&
            chart_timeline_controls_date_format_display %in% names(named_list)
        ) {
          if (is.logical(chart_timeline_controls_date_format_display)) {
            chart_timeline_controls_date_format_display <- as.character(
