...
#' @name set_svg_annotator_details
#' @param svg_url SVG file. Flourish type hint: url, defaults to `NULL`
#' @param svg_width_wide Image width. The width in % of the image in standard wide-screen view. Flourish type hint: number, defaults to `NULL`
...
#' @param mobile_svg_width Width. The width in % of the image on a mobile view. Flourish type hint: number, defaults to `NULL`
...
#' @param clickable_svg_animation_config Advanced. Reveal settings to adjust the SVG Animation properties on clickable elements. See https://developer.mozilla.org/en-US/docs/Web/SVG/Element/animate for more details. Flourish type hint: boolean, defaults to `NULL`
...
#' @param text_panel_enabled Text panel enabled. Set to disabled to only show text in popups. Flourish type hint: boolean, defaults to `NULL`
#' @param h1_default Default heading. The text to show on load. Flourish type hint: text, defaults to `NULL`
#' @param h1_size Size. Flourish type hint: number, defaults to `NULL`
#' @param h2_default Default subheading. The text to show on load. Flourish type hint: text, defaults to `NULL`
#' @param h2_size Size. Flourish type hint: number, defaults to `NULL`
#' @param content_default Default subheading. The text to show on load. Flourish type hint: text, defaults to `NULL`
...
#' @param chart_popup_panel_vertical_position Vertical position. Choices: click, center. Choosing "Click position" is especially useful when your visualisation is really tall, this will make sure the panel content is always on top of the element youve clicked on. Flourish type hint: string, defaults to `NULL`
...
#' @param stylesheet Custom styles. Use the class .clickable-element to target all interactive elements. When one is clicked, it is .highlighted and the others are .faded. The text for each clickable element is inside a .text-block, which is also .accordion in accordion mode. Flourish type hint: code, defaults to `NULL`
#' @param chart_layout_body_font Main font. This font will apply to the whole graphic by default, but you can optionally change the font for the title, subtitle, footer, etc in the Header and Footer settings panels. Flourish type hint: font, defaults to `NULL`
#' @param chart_layout_font_color Text color. This color will apply to the whole graphic by default, but you can optionally change the color for individual text elements, in other settings panels. Flourish type hint: color, defaults to `NULL`
...
#' @param chart_layout_layout_order Layout order. Choices: stack-default, stack-2, stack-3, stack-4. Flourish type hint: string, defaults to `NULL`
...
#' @param chart_layout_title_size_custom Custom. Specify a custom responsive font size. Best results will be with values between 1.2 and 3. Flourish type hint: number, defaults to `NULL`
...
#' @param chart_layout_subtitle_size_custom Custom. Specify a custom responsive font size. Best results will be with values between 1.2 and 3. Flourish type hint: number, defaults to `NULL`
...
#' @param chart_layout_header_text_size_custom Custom. Specify a custom responsive font size. Best results will be with values between 1.2 and 3. Flourish type hint: number, defaults to `NULL`
...
#' @param chart_layout_header_logo_align Align. Choices: inside, outside. Align logo inside either the header or the main visualisation container. Flourish type hint: string, defaults to `NULL`
...
#' @param chart_layout_footer_note Note. Flourish type hint: string, defaults to `NULL`
#' @param chart_layout_footer_note_secondary Note (secondary). The secondary note is placed below the source and primary note. Flourish type hint: string, defaults to `NULL`
...
#' @param chart_layout_footer_logo_src_light Image (light version). If provided this version will be used whenever the background colour is dark. Flourish type hint: hidden, defaults to `NULL`
...
set_svg_annotator_details <- function(
  .,
  svg_url = NULL,
  svg_width_wide = NULL,
  mobile_view = NULL,
  ...
  details_error(., "svg_annotator")

  old_list <- .
  new_list <- list()

  if (isTRUE(old_list$x$template != "@flourish/svg-annotator")) {
    stop("`set_svg_annotator_details()` is not supported for this chart type.")
  }
    ...
    if (!mobile_svg_mode %in% list_mobile_svg_mode) {
      ...
      if (
        length(jsonlite::fromJSON(
          '{"none": "none", "same": "same", "different": "different"}'
        )) >
          1
      ) {
  ...
  if (!is.null(mobile_content_mode)) {
    if (!mobile_content_mode %in% list_mobile_content_mode) {
      stop(
        "`mobile_content_mode` must be one of: c('interactive', 'all', 'accordion')"
      )
    } else {
      if (
        length(jsonlite::fromJSON(
          '{"interactive": "interactive", "all": "all", "accordion": "accordion"}'
        )) >
          1
      ) {
        named_list <- jsonlite::fromJSON(
        ...
        names(named_list) <- boolean_names
        if (
          length(named_list) > 1 && mobile_content_mode %in% names(named_list)
        ) {
  ...
  if (!is.null(clickable_svg_animation)) {
    if (!clickable_svg_animation %in% list_clickable_svg_animation) {
      ...
      if (
        length(jsonlite::fromJSON('{"off": "off", "animation": "animation"}')) >
          1
      ) {
        named_list <- jsonlite::fromJSON(
          '{"off": "off", "animation": "animation"}'
        )
        # Get the names of the list
        names_list <- names(named_list)
        # Convert names to boolean values only if they are "true" or "false"
        ...
        if (
          length(named_list) > 1 &&
            clickable_svg_animation %in% names(named_list)
        ) {
  ...
  if (length(h1_default) > 1) {
    new_list$x$state$h1_default <- paste(h1_default, collapse = "\n")
  } else {
    new_list$x$state$h1_default <- h1_default
  }

  new_list$x$state$h1_size <- h1_size
  if (length(h2_default) > 1) {
    new_list$x$state$h2_default <- paste(h2_default, collapse = "\n")
  } else {
    new_list$x$state$h2_default <- h2_default
  }

  new_list$x$state$h2_size <- h2_size
  ...
  list_chart_layout_layout_order <- c(
    "stack-default",
    "stack-2",
    "stack-3",
    "stack-4",
  ...
  if (!is.null(chart_layout_layout_order)) {
    if (!chart_layout_layout_order %in% list_chart_layout_layout_order) {
      stop(
        "`chart_layout_layout_order` must be one of: c('stack-default', 'stack-2', 'stack-3', 'stack-4')"
      )
    } else {
      if (
        length(jsonlite::fromJSON(
          '{"stack-default": "stack-default", "stack-2": "stack-2", "stack-3": "stack-3", "stack-4": "stack-4"}'
        )) >
          1
      ) {
        named_list <- jsonlite::fromJSON(
        ...
        if (
          length(named_list) > 1 &&
            chart_layout_layout_order %in% names(named_list)
        ) {
          if (is.logical(chart_layout_layout_order)) {
            chart_layout_layout_order <- as.character(chart_layout_layout_order)
          }
          named_list_value <- unname(named_list[chart_layout_layout_order])[[1]]
          new_list$x$state$layout$layout_order <- named_list_value
        }
      }
    }
  }
