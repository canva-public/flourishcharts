...
set_svg_annotator_details <- function(
  ...
  list_chart_layout_layout_order <- c("stack-default", "stack-2", "stack-3", "stack-4", NULL)
  if (!is.null(chart_layout_layout_order)) {
    if (!chart_layout_layout_order %in% list_chart_layout_layout_order) {
      stop("`chart_layout_layout_order` must be one of: c('stack-default', 'stack-2', 'stack-3', 'stack-4')")
    } else {
      if (length(jsonlite::fromJSON('{"stack-default": "stack-default", "stack-2": "stack-2", "stack-3": "stack-3", "stack-4": "stack-4"}')) > 1) {
        ...
        # Set the names of the list to boolean values
        names(named_list) <- boolean_names
        if (length(named_list) > 1 && chart_layout_layout_order %in% names(named_list)) {
