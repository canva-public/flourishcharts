...
#' @name set_text_annotator_details
#' @param text Text. Fill in the text you want to annotate In the datasheet, you can write which parts of the text should be annotated. If you want to annotate a specific part of the text that occurs more than once, you can add identifiers to your text. For example, annotating the word beautiful would look like: `[beautiful]{beautiful-annotation}`. Then, in the datasheet, you can write beautiful-annotation in the first column and the text to include in the annotation in the second column. Flourish type hint: html, defaults to `NULL`
#' @param highlight_color Highlight color. Flourish type hint: color, defaults to `NULL`
#' @param layout_align Align. Choices: center, start. Flourish type hint: string, defaults to `NULL`
#' @param text_container_width Width. Max width of text container. Flourish type hint: number, defaults to `NULL`
#' @param text_font_size Font size. In rems, a multiple of the pages base font size. Flourish type hint: number, defaults to `NULL`
#' @param text_font_color Text color. Flourish type hint: color, defaults to `NULL`
#' @param text_font_color_highlighted Text color (highlighted). Flourish type hint: color, defaults to `NULL`
#' @param text_font_color_highlighted_contrast Text color (highlighted). Flourish type hint: color, defaults to `NULL`
#' @param text_highlight_type Highlight type. Choices: handdrawn, background, text_color, underline. Flourish type hint: string, defaults to `NULL`
#' @param text_highlight_handdrawn Handdrawn type. Choices: circle, highlight, box, underline. Flourish type hint: string, defaults to `NULL`
#' @param highlighted_stroke_end Stroke end. Choices: round,Flourish type hint: string, defaults to `NULL`
#' @param highlighted_stroke_amount Iterations. Number of lines to draw. Flourish type hint: number, defaults to `NULL`
#' @param highlighted_stroke_width Thickness. Flourish type hint: number, defaults to `NULL`
#' @param note_type Mode. Choices: side-panel, inline. Flourish type hint: string, defaults to `NULL`
#' @param note_visibility Show notes. Choices: always, click. Flourish type hint: string, defaults to `NULL`
#' @param note_font_size Text size. Flourish type hint: number, defaults to `NULL`
#' @param side_panel_width Width. Max width of side panel. Flourish type hint: number, defaults to `NULL`
#' @param note_font_color Text color. Flourish type hint: color, defaults to `NULL`
#' @param note_background_color Background color. Flourish type hint: color, defaults to `NULL`
#' @param note_custom_content Note content. Use `{{NOTE}}` to show the annotation content. Use this setting to add prefixes, suffixes or additional HTML styling to your annotations. Flourish type hint: html, defaults to `NULL`
#' @param note_mobile_mode Mobile mode. Choices: side-panel, inline, overlay. Choose where to display the notes on smaller screens. Flourish type hint: string, defaults to `NULL`
...
set_text_annotator_details <- function(
  .,
  text = NULL,
  ...
  details_error(., "text_annotator")

  old_list <- .
  new_list <- list()

  if (isTRUE(old_list$x$template != "@flourish/text-annotator")) {
    stop("`set_text_annotator_details()` is not supported for this chart type.")
  }
  if (length(text) > 1) {
    new_list$x$state$text <- paste(text, collapse = "\n")
  } else {
    new_list$x$state$text <- text
  }

  new_list$x$state$highlight_color <- highlight_color
  list_layout_align <- c("center", "start", NULL)
  if (!is.null(layout_align)) {
    if (!layout_align %in% list_layout_align) {
      stop("`layout_align` must be one of: c('center', 'start')")
    } else {
      if (
        length(jsonlite::fromJSON('{"center": "center", "start": "start"}')) > 1
      ) {
        ...
        if (length(named_list) > 1 && layout_align %in% names(named_list)) {
          if (is.logical(layout_align)) {
            layout_align <- as.character(layout_align)
          }
          named_list_value <- unname(named_list[layout_align])[[1]]
          new_list$x$state$layout_align <- named_list_value
        }
      }
    }
  }
  new_list$x$state$text_container_width <- text_container_width
  new_list$x$state$text_font_size <- text_font_size
  new_list$x$state$text_font_color <- text_font_color
  new_list$x$state$text_font_color_highlighted <- text_font_color_highlighted
  new_list$x$state$text_font_color_highlighted_contrast <- text_font_color_highlighted_contrast
  list_text_highlight_type <- c(
    "handdrawn",
    "background",
    "text_color",
    "underline",
  ...
  if (!is.null(text_highlight_type)) {
    if (!text_highlight_type %in% list_text_highlight_type) {
      stop(
        "`text_highlight_type` must be one of: c('handdrawn', 'background', 'text_color', 'underline')"
      )
    } else {
      if (
        length(jsonlite::fromJSON(
          '{"handdrawn": "handdrawn", "background": "background", "text_color": "text_color", "underline": "underline"}'
        )) >
          1
      ) {
        named_list <- jsonlite::fromJSON(
        ...
        names(named_list) <- boolean_names
        if (
          length(named_list) > 1 && text_highlight_type %in% names(named_list)
        ) {
          if (is.logical(text_highlight_type)) {
            text_highlight_type <- as.character(text_highlight_type)
          }
          named_list_value <- unname(named_list[text_highlight_type])[[1]]
          new_list$x$state$text_highlight_type <- named_list_value
        }
      }
    }
  }
  list_text_highlight_handdrawn <- c(
    "circle",
    "highlight",
    "box",
    "underline",
  ...
  if (!is.null(text_highlight_handdrawn)) {
    if (!text_highlight_handdrawn %in% list_text_highlight_handdrawn) {
      stop(
        "`text_highlight_handdrawn` must be one of: c('circle', 'highlight', 'box', 'underline')"
      )
    } else {
      if (
        length(jsonlite::fromJSON(
          '{"circle": "circle", "highlight": "highlight", "box": "box", "underline": "underline"}'
        )) >
          1
      ) {
        named_list <- jsonlite::fromJSON(
        ...
        if (
          length(named_list) > 1 &&
            text_highlight_handdrawn %in% names(named_list)
        ) {
          if (is.logical(text_highlight_handdrawn)) {
            text_highlight_handdrawn <- as.character(text_highlight_handdrawn)
          }
          named_list_value <- unname(named_list[text_highlight_handdrawn])[[1]]
          new_list$x$state$text_highlight_handdrawn <- named_list_value
        }
      }
    }
  }
  list_highlighted_stroke_end <- c("round", "", NULL)
  if (!is.null(highlighted_stroke_end)) {
    if (!highlighted_stroke_end %in% list_highlighted_stroke_end) {
      stop("`highlighted_stroke_end` must be one of: c('round', '')")
    } else {
      if (length(jsonlite::fromJSON('{"round": "round", "": ""}')) > 1) {
        named_list <- jsonlite::fromJSON('{"round": "round", "": ""}')
        # Get the names of the list
        names_list <- names(named_list)
        # Convert names to boolean values only if they are "true" or "false"
        boolean_names <- sapply(names_list, function(x) {
        ...
        if (
          length(named_list) > 1 &&
            highlighted_stroke_end %in% names(named_list)
        ) {
          if (is.logical(highlighted_stroke_end)) {
            highlighted_stroke_end <- as.character(highlighted_stroke_end)
          }
          named_list_value <- unname(named_list[highlighted_stroke_end])[[1]]
          new_list$x$state$highlighted_stroke_end <- named_list_value
        }
      }
    }
  }
  new_list$x$state$highlighted_stroke_amount <- highlighted_stroke_amount
  new_list$x$state$highlighted_stroke_width <- highlighted_stroke_width
  list_note_type <- c("side-panel", "inline", NULL)
  if (!is.null(note_type)) {
    if (!note_type %in% list_note_type) {
      stop("`note_type` must be one of: c('side-panel', 'inline')")
    } else {
      if (
        length(jsonlite::fromJSON(
          '{"side-panel": "side-panel", "inline": "inline"}'
        )) >
          1
      ) {
        named_list <- jsonlite::fromJSON(
        ...
        # Set the names of the list to boolean values
        names(named_list) <- boolean_names
        if (length(named_list) > 1 && note_type %in% names(named_list)) {
          if (is.logical(note_type)) {
            note_type <- as.character(note_type)
          }
          named_list_value <- unname(named_list[note_type])[[1]]
          new_list$x$state$note_type <- named_list_value
        }
      }
    }
  }
  list_note_visibility <- c("always", "click", NULL)
  if (!is.null(note_visibility)) {
    if (!note_visibility %in% list_note_visibility) {
      stop("`note_visibility` must be one of: c('always', 'click')")
    } else {
      if (
        length(jsonlite::fromJSON('{"always": "always", "click": "click"}')) > 1
      ) {
        named_list <- jsonlite::fromJSON(
          '{"always": "always", "click": "click"}'
        )
        # Get the names of the list
        names_list <- names(named_list)
        # Convert names to boolean values only if they are "true" or "false"
        ...
        # Set the names of the list to boolean values
        names(named_list) <- boolean_names
        if (length(named_list) > 1 && note_visibility %in% names(named_list)) {
          if (is.logical(note_visibility)) {
            note_visibility <- as.character(note_visibility)
          }
          named_list_value <- unname(named_list[note_visibility])[[1]]
          new_list$x$state$note_visibility <- named_list_value
        }
      }
    }
  }
  new_list$x$state$note_font_size <- note_font_size
  new_list$x$state$side_panel_width <- side_panel_width
  new_list$x$state$note_font_color <- note_font_color
  new_list$x$state$note_background_color <- note_background_color
  if (length(note_custom_content) > 1) {
    new_list$x$state$note_custom_content <- paste(
      note_custom_content,
      collapse = "\n"
    )
  } else {
    new_list$x$state$note_custom_content <- note_custom_content
  }

  list_note_mobile_mode <- c("side-panel", "inline", "overlay", NULL)
  if (!is.null(note_mobile_mode)) {
    if (!note_mobile_mode %in% list_note_mobile_mode) {
      stop(
        "`note_mobile_mode` must be one of: c('side-panel', 'inline', 'overlay')"
      )
    } else {
      if (
        length(jsonlite::fromJSON(
          '{"side-panel": "side-panel", "inline": "inline", "overlay": "overlay"}'
        )) >
          1
      ) {
        named_list <- jsonlite::fromJSON(
        ...
        # Set the names of the list to boolean values
        names(named_list) <- boolean_names
        if (length(named_list) > 1 && note_mobile_mode %in% names(named_list)) {
          if (is.logical(note_mobile_mode)) {
            note_mobile_mode <- as.character(note_mobile_mode)
          }
          named_list_value <- unname(named_list[note_mobile_mode])[[1]]
          new_list$x$state$note_mobile_mode <- named_list_value
        }
      }
    }
  }
