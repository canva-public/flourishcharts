...
#' @name set_tournament_details
#' @param min_round_width_mode Width mode. Choices: auto, fixed. Auto mode sets round width to the smallest it can be while still fitting text content. In Fixed mode you can choose a fixed minimum round width (text will be ellipsed if it does not fit). Flourish type hint: string, defaults to `NULL`
#' @param min_round_width_fixed Min stage width. The minimum width given to a specific stage (e.g. Quarterfinals), specified in px. Flourish type hint: number, defaults to `NULL`
#' @param inner_padding_vertical Box height. Flourish type hint: number, defaults to `NULL`
#' @param inner_padding_horizontal Box padding. Horizontal space on sides of box. Flourish type hint: number, defaults to `NULL`
#' @param link_width Horizontal space. Horizontal space between stages. Flourish type hint: number, defaults to `NULL`
#' @param outer_padding Vertical space. Vertical space between stages. Flourish type hint: number, defaults to `NULL`
#' @param winning_team_color Winning team. Flourish type hint: color, defaults to `NULL`
#' @param losing_team_color Losing team. Flourish type hint: color, defaults to `NULL`
#' @param upcoming_fill_color Upcoming match. Flourish type hint: color, defaults to `NULL`
#' @param boxes_stroke_color Outline color. Flourish type hint: color, defaults to `NULL`
#' @param boxes_stroke_width Outline width. Flourish type hint: number, defaults to `NULL`
#' @param boxes_roundness Roundness. Flourish type hint: number, defaults to `NULL`
#' @param winning_team_text_color Winning team. Flourish type hint: color, defaults to `NULL`
#' @param losing_team_text_color Losing team. Flourish type hint: color, defaults to `NULL`
#' @param font_size Size. In rems, a multiple of the pages base font size. Flourish type hint: number, defaults to `NULL`
#' @param text_weight_mode Weight. Choices: winner_bold, regular, bold. Flourish type hint: string, defaults to `NULL`
#' @param link_thickness Thickness. In rems, a multiple of the pages base font size. Flourish type hint: number, defaults to `NULL`
#' @param link_color Color. Flourish type hint: color, defaults to `NULL`
#' @param link_radius Roundness. In rems, a multiple of the pages base font size. Flourish type hint: number, defaults to `NULL`
#' @param image_size Size. Specified as a % of the box for that match. Flourish type hint: number, defaults to `NULL`
#' @param image_scale Scale. Choices: contain, cover, zoom. Fill will not preserve the aspect ratio of images but stretch them to fill the area.  Fit and Zoom both preserve the aspect ratio of images, but Fit ensures none of the image is cut off. Flourish type hint: string, defaults to `NULL`
...
#' @param headings_enabled Choices: `TRUE`, `FALSE`. Flourish type hint: boolean, defaults to `NULL`
#' @param headings_size Size. In rems, a multiple of the pages base font size. Flourish type hint: number, defaults to `NULL`
#' @param headings_padding Padding. In rems, a multiple of the pages base font size. Flourish type hint: number, defaults to `NULL`
#' @param headings_color Color. Flourish type hint: color, defaults to `NULL`
#' @param headings_weight Weight. Choices: bold, normal. Flourish type hint: string, defaults to `NULL`
#' @param headings_position Position. Choices: middle, start. Flourish type hint: string, defaults to `NULL`
#' @param navigation_icon_position Position. Choices: start, center, end. Flourish type hint: string, defaults to `NULL`
#' @param navigation_icon_mode Type. Choices: arrow, custom. Flourish type hint: string, defaults to `NULL`
#' @param navigation_icon_upload Upload icon. Upload a right-facing icon (your icon will be rotated for the left side). Flourish type hint: url, defaults to `NULL`
#' @param navigation_icon_size Size. In rems, a multiple of the pages base font size. Flourish type hint: number, defaults to `NULL`
...
#' @param popup_header_separator Header separator. Flourish type hint: string, defaults to `NULL`
#' @param animation_duration Duration. The duration, in seconds, of intro animation. Flourish type hint: number, defaults to `NULL`
...
set_tournament_details <- function(
    .,
  ...
  details_error(., "tournament")

  old_list <- .
  new_list <- list()

  if (isTRUE(old_list$x$template != "@flourish/tournament")) {
    stop("`set_tournament_details()` is not supported for this chart type.")
  }
  list_min_round_width_mode <- c("auto", "fixed", NULL)
  if (!is.null(min_round_width_mode)) {
    if (!min_round_width_mode %in% list_min_round_width_mode) {
      stop("`min_round_width_mode` must be one of: c('auto', 'fixed')")
    } else {
      if (length(jsonlite::fromJSON('{"auto": "auto", "fixed": "fixed"}')) > 1) {
        ...
        if (length(named_list) > 1 && min_round_width_mode %in% names(named_list)) {
          if (is.logical(min_round_width_mode)) {
            min_round_width_mode <- as.character(min_round_width_mode)
          }
          named_list_value <- unname(named_list[min_round_width_mode])[[1]]
          new_list$x$state$min_round_width_mode <- named_list_value
        }
      }
    }
  }
  new_list$x$state$min_round_width_fixed <- min_round_width_fixed
  new_list$x$state$inner_padding_vertical <- inner_padding_vertical
  new_list$x$state$inner_padding_horizontal <- inner_padding_horizontal
  new_list$x$state$link_width <- link_width
  new_list$x$state$outer_padding <- outer_padding
  new_list$x$state$winning_team_color <- winning_team_color
  new_list$x$state$losing_team_color <- losing_team_color
  new_list$x$state$upcoming_fill_color <- upcoming_fill_color
  new_list$x$state$boxes_stroke_color <- boxes_stroke_color
  new_list$x$state$boxes_stroke_width <- boxes_stroke_width
  new_list$x$state$boxes_roundness <- boxes_roundness
  new_list$x$state$winning_team_text_color <- winning_team_text_color
  new_list$x$state$losing_team_text_color <- losing_team_text_color
  new_list$x$state$font_size <- font_size
  list_text_weight_mode <- c("winner_bold", "regular", "bold", NULL)
  if (!is.null(text_weight_mode)) {
    if (!text_weight_mode %in% list_text_weight_mode) {
      stop("`text_weight_mode` must be one of: c('winner_bold', 'regular', 'bold')")
    } else {
      if (length(jsonlite::fromJSON('{"winner_bold": "winner_bold", "regular": "regular", "bold": "bold"}')) > 1) {
        named_list <- jsonlite::fromJSON('{"winner_bold": "winner_bold", "regular": "regular", "bold": "bold"}')
        # Get the names of the list
        names_list <- names(named_list)
        # Convert names to boolean values only if they are "true" or "false"
        boolean_names <- sapply(names_list, function(x) {
        ...
        # Set the names of the list to boolean values
        names(named_list) <- boolean_names
        if (length(named_list) > 1 && text_weight_mode %in% names(named_list)) {
          if (is.logical(text_weight_mode)) {
            text_weight_mode <- as.character(text_weight_mode)
          }
          named_list_value <- unname(named_list[text_weight_mode])[[1]]
          new_list$x$state$text_weight_mode <- named_list_value
        }
      }
    }
  }
  new_list$x$state$link_thickness <- link_thickness
  new_list$x$state$link_color <- link_color
  new_list$x$state$link_radius <- link_radius
  new_list$x$state$image_size <- image_size
  list_image_scale <- c("contain", "cover", "zoom", NULL)
  if (!is.null(image_scale)) {
    if (!image_scale %in% list_image_scale) {
      stop("`image_scale` must be one of: c('contain', 'cover', 'zoom')")
    } else {
  ...
  list_headings_enabled <- c(TRUE, FALSE, NULL)
  if (!is.null(headings_enabled)) {
    if (!headings_enabled %in% list_headings_enabled) {
      stop("`headings_enabled` must be one of: c(TRUE, FALSE)")
    } else {
      if (length(jsonlite::fromJSON('{"true": true, "false": false}')) > 1) {
        ...
        # Set the names of the list to boolean values
        names(named_list) <- boolean_names
        if (length(named_list) > 1 && headings_enabled %in% names(named_list)) {
          if (is.logical(headings_enabled)) {
            headings_enabled <- as.character(headings_enabled)
          }
          named_list_value <- unname(named_list[headings_enabled])[[1]]
          new_list$x$state$headings_enabled <- named_list_value
        }
      }
    }
  }
  new_list$x$state$headings_size <- headings_size
  new_list$x$state$headings_padding <- headings_padding
  new_list$x$state$headings_color <- headings_color
  list_headings_weight <- c("bold", "normal", NULL)
  if (!is.null(headings_weight)) {
    if (!headings_weight %in% list_headings_weight) {
      stop("`headings_weight` must be one of: c('bold', 'normal')")
    } else {
      if (length(jsonlite::fromJSON('{"bold": "bold", "normal": "normal"}')) > 1) {
        ...
        # Set the names of the list to boolean values
        names(named_list) <- boolean_names
        if (length(named_list) > 1 && headings_weight %in% names(named_list)) {
          if (is.logical(headings_weight)) {
            headings_weight <- as.character(headings_weight)
          }
          named_list_value <- unname(named_list[headings_weight])[[1]]
          new_list$x$state$headings_weight <- named_list_value
        }
      }
    }
  }
  list_headings_position <- c("middle", "start", NULL)
  if (!is.null(headings_position)) {
    if (!headings_position %in% list_headings_position) {
      stop("`headings_position` must be one of: c('middle', 'start')")
    } else {
      if (length(jsonlite::fromJSON('{"middle": "middle", "start": "start"}')) > 1) {
        named_list <- jsonlite::fromJSON('{"middle": "middle", "start": "start"}')
        # Get the names of the list
        names_list <- names(named_list)
        # Convert names to boolean values only if they are "true" or "false"
        boolean_names <- sapply(names_list, function(x) {
        ...
        # Set the names of the list to boolean values
        names(named_list) <- boolean_names
        if (length(named_list) > 1 && headings_position %in% names(named_list)) {
          if (is.logical(headings_position)) {
            headings_position <- as.character(headings_position)
          }
          named_list_value <- unname(named_list[headings_position])[[1]]
          new_list$x$state$headings_position <- named_list_value
        }
      }
    }
  }
  list_navigation_icon_position <- c("start", "center", "end", NULL)
  if (!is.null(navigation_icon_position)) {
    if (!navigation_icon_position %in% list_navigation_icon_position) {
      stop("`navigation_icon_position` must be one of: c('start', 'center', 'end')")
    } else {
      if (length(jsonlite::fromJSON('{"start": "start", "center": "center", "end": "end"}')) > 1) {
        ...
        # Set the names of the list to boolean values
        names(named_list) <- boolean_names
        if (length(named_list) > 1 && navigation_icon_position %in% names(named_list)) {
          if (is.logical(navigation_icon_position)) {
            navigation_icon_position <- as.character(navigation_icon_position)
          }
          named_list_value <- unname(named_list[navigation_icon_position])[[1]]
          new_list$x$state$navigation_icon_position <- named_list_value
        }
      }
    }
  }
  list_navigation_icon_mode <- c("arrow", "custom", NULL)
  if (!is.null(navigation_icon_mode)) {
    if (!navigation_icon_mode %in% list_navigation_icon_mode) {
      stop("`navigation_icon_mode` must be one of: c('arrow', 'custom')")
    } else {
      if (length(jsonlite::fromJSON('{"arrow": "arrow", "custom": "custom"}')) > 1) {
        named_list <- jsonlite::fromJSON('{"arrow": "arrow", "custom": "custom"}')
        # Get the names of the list
        names_list <- names(named_list)
        # Convert names to boolean values only if they are "true" or "false"
        boolean_names <- sapply(names_list, function(x) {
        ...
        # Set the names of the list to boolean values
        names(named_list) <- boolean_names
        if (length(named_list) > 1 && navigation_icon_mode %in% names(named_list)) {
          if (is.logical(navigation_icon_mode)) {
            navigation_icon_mode <- as.character(navigation_icon_mode)
          }
          named_list_value <- unname(named_list[navigation_icon_mode])[[1]]
          new_list$x$state$navigation_icon_mode <- named_list_value
        }
      }
    }
  }
  new_list$x$state$navigation_icon_upload <- navigation_icon_upload
  ...
  new_list$x$state$popup_header_separator <- popup_header_separator
