...
set_tournament_details <- function(
  ...
  if (!is.null(min_round_width_mode)) {
    if (!min_round_width_mode %in% list_min_round_width_mode) {
      ...
      if (
        ...
        if (
          length(named_list) > 1 && min_round_width_mode %in% names(named_list)
        ) {
  ...
  if (!is.null(text_weight_mode)) {
    if (!text_weight_mode %in% list_text_weight_mode) {
      stop(
        "`text_weight_mode` must be one of: c('winner_bold', 'regular', 'bold')"
      )
    } else {
      if (
        length(jsonlite::fromJSON(
          '{"winner_bold": "winner_bold", "regular": "regular", "bold": "bold"}'
        )) >
          1
      ) {
  ...
  if (!is.null(image_scale)) {
    if (!image_scale %in% list_image_scale) {
      ...
      if (
        length(jsonlite::fromJSON(
          '{"contain": "contain", "cover": "cover", "zoom": "zoom"}'
        )) >
          1
      ) {
        named_list <- jsonlite::fromJSON(
  ...
  if (!is.null(image_shape)) {
    if (!image_shape %in% list_image_shape) {
      ...
      if (
        length(jsonlite::fromJSON('{"circle": "circle", "square": "square"}')) >
          1
      ) {
        named_list <- jsonlite::fromJSON(
  ...
  if (!is.null(headings_position)) {
    if (!headings_position %in% list_headings_position) {
      ...
      if (
        length(jsonlite::fromJSON('{"middle": "middle", "start": "start"}')) > 1
      ) {
        named_list <- jsonlite::fromJSON(
          '{"middle": "middle", "start": "start"}'
        )
        # Get the names of the list
        names_list <- names(named_list)
        # Convert names to boolean values only if they are "true" or "false"
        ...
        names(named_list) <- boolean_names
        if (
          length(named_list) > 1 && headings_position %in% names(named_list)
        ) {
  ...
  if (!is.null(navigation_icon_position)) {
    if (!navigation_icon_position %in% list_navigation_icon_position) {
      stop(
        "`navigation_icon_position` must be one of: c('start', 'center', 'end')"
      )
    } else {
      if (
        length(jsonlite::fromJSON(
        ...
        if (
          length(named_list) > 1 &&
            navigation_icon_position %in% names(named_list)
        ) {
  ...
  if (!is.null(navigation_icon_mode)) {
    if (!navigation_icon_mode %in% list_navigation_icon_mode) {
      ...
      if (
        length(jsonlite::fromJSON('{"arrow": "arrow", "custom": "custom"}')) > 1
      ) {
        named_list <- jsonlite::fromJSON(
          '{"arrow": "arrow", "custom": "custom"}'
        )
        # Get the names of the list
        names_list <- names(named_list)
        # Convert names to boolean values only if they are "true" or "false"
        ...
        names(named_list) <- boolean_names
        if (
          length(named_list) > 1 && navigation_icon_mode %in% names(named_list)
        ) {
