...
set_word_cloud_details <- function(
  ...
  if (!is.null(size_mode)) {
    if (!size_mode %in% list_size_mode) {
      ...
      if (length(jsonlite::fromJSON('{"linear": "linear", "sqrt": "sqrt", "log": "log"}')) > 1) {
        named_list <- jsonlite::fromJSON('{"linear": "linear", "sqrt": "sqrt", "log": "log"}')
        # Get the names of the list
        names_list <- names(named_list)
        # Convert names to boolean values only if they are "true" or "false"
        boolean_names <- sapply(names_list, function(x) {
  ...
  if (!is.null(animation_type)) {
    if (!animation_type %in% list_animation_type) {
      ...
      if (length(jsonlite::fromJSON('{"auto": "auto", "fade": "fade", "grow": "grow", "none": "none"}')) > 1) {
        named_list <- jsonlite::fromJSON('{"auto": "auto", "fade": "fade", "grow": "grow", "none": "none"}')
        # Get the names of the list
        names_list <- names(named_list)
        # Convert names to boolean values only if they are "true" or "false"
        boolean_names <- sapply(names_list, function(x) {
