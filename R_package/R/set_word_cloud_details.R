...
#' @name set_word_cloud_details
#' @param using_tables Text input. Choices: `FALSE`, `TRUE`. Pasting in raw text is simplest but may be slow for large amounts of text. Flourish type hint: boolean, defaults to `NULL`
#' @param word_limit Word limit. Limits the number of words, in order of frequency. Leave blank to show all words in the data table. Flourish type hint: number, defaults to `NULL`
#' @param raw_text Text. Note that the raw text will be available in the HTML of the generated page. This occurs when using both Paste in raw text and Datasheet options. Flourish type hint: text, defaults to `NULL`
#' @param exclusions Exclude common words. Flourish type hint: boolean, defaults to `NULL`
#' @param excluded_words Exclude words. Flourish type hint: text, defaults to `NULL`
#' @param strip_trailing_s Strip all trailing "s" from words. e.g. "mother" and "mothers" are considered one word. This includes both apostrophe and quotation mark forms (s and s). Flourish type hint: boolean, defaults to `NULL`
#' @param convert_to_lowercase Convert all words to lowercase. e.g. "mother" and "Mother" are considered one word. This will also apply to the words in the Exclude words setting. Flourish type hint: boolean, defaults to `NULL`
#' @param cloud_font Font. Flourish type hint: font, defaults to `NULL`
#' @param text_color Text color. Flourish type hint: color, defaults to `NULL`
#' @param word_padding Spacing. Spacing between the words (in arbitrary units). Flourish type hint: number, defaults to `NULL`
#' @param random_seed Layout number. Change the number to try different word layouts. Flourish type hint: number, defaults to `NULL`
#' @param angle_min Min angle. Flourish type hint: number, defaults to `NULL`
#' @param angle_max Max angle. Flourish type hint: number, defaults to `NULL`
#' @param angle_number Number of angles. Flourish type hint: number, defaults to `NULL`
#' @param font_size_min Min size. In arbitrary units. Actual display size will depend in screen size and layout. Flourish type hint: number, defaults to `NULL`
#' @param font_size_max Max size. In arbitrary units. Actual display size will depend in screen size and layout. Flourish type hint: number, defaults to `NULL`
#' @param size_mode Scaling. Choices: linear, sqrt, log. This scale will determine the relationship in size between the biggest and the smallest word. Flourish type hint: string, defaults to `NULL`
#' @param height_mode Height mode. Choices: fill_space, aspect. In Fill space mode, the graphic will fill the container (which by default will be the standard Flourish responsive chart size). In Aspect ratio mode, you set the aspect ratio of the plot, and the container will be updated to accommodate it.  This setting is ignored when embedded in a fixed-height context, such as a simple iframe embedFlourish type hint: string, defaults to `NULL`
#' @param aspect_ratio Aspect (desktop). Aspect ratio of each plot in desktop mode, where 1 is square, above 1 is wider, and below 1 is taller. Flourish type hint: number, defaults to `NULL`
#' @param aspect_ratio_mobile Aspect (mobile). Aspect ratio of each plot in mobile mode, where 1 is square, above 1 is wider, and below 1 is taller. Flourish type hint: number, defaults to `NULL`
...
#' @param animation_type Animation style. Choices: auto, fade, grow, none. Controls the animation on load.  Auto mode will use Grow if there are fewer than 100 words visible, Fade if there are fewer than 500, and None if there are 500 or more. Flourish type hint: string, defaults to `NULL`
#' @param duration Duration. The time in seconds it takes for an individual word to move from the start position to end position. Flourish type hint: number, defaults to `NULL`
#' @param stagger Stagger. The time in seconds between the first and last word animations starting. Flourish type hint: number, defaults to `NULL`
...
set_word_cloud_details <- function(
  .,
  using_tables = NULL,
  word_limit = NULL,
  raw_text = NULL,
  ...
  details_error(., "word_cloud")

  old_list <- .
  new_list <- list()

  if (isTRUE(old_list$x$template != "@flourish/word-cloud")) {
    stop("`set_word_cloud_details()` is not supported for this chart type.")
  }
  list_using_tables <- c(FALSE, TRUE, NULL)
  if (!is.null(using_tables)) {
    if (!using_tables %in% list_using_tables) {
      stop("`using_tables` must be one of: c(FALSE, TRUE)")
    } else {
      if (length(jsonlite::fromJSON('{"false": false, "true": true}')) > 1) {
        ...
        if (length(named_list) > 1 && using_tables %in% names(named_list)) {
          if (is.logical(using_tables)) {
            using_tables <- as.character(using_tables)
          }
          named_list_value <- unname(named_list[using_tables])[[1]]
          new_list$x$state$using_tables <- named_list_value
        }
      }
    }
  }
  new_list$x$state$word_limit <- word_limit
  if (length(raw_text) > 1) {
    new_list$x$state$raw_text <- paste(raw_text, collapse = "\n")
  } else {
    new_list$x$state$raw_text <- raw_text
  }

  new_list$x$state$exclusions <- exclusions
  if (length(excluded_words) > 1) {
    new_list$x$state$excluded_words <- paste(excluded_words, collapse = "\n")
  } else {
    new_list$x$state$excluded_words <- excluded_words
  }

  new_list$x$state$strip_trailing_s <- strip_trailing_s
  new_list$x$state$convert_to_lowercase <- convert_to_lowercase
  new_list$x$state$cloud_font <- cloud_font
  new_list$x$state$text_color <- text_color
  new_list$x$state$word_padding <- word_padding
  new_list$x$state$random_seed <- random_seed
  new_list$x$state$angle_min <- angle_min
  new_list$x$state$angle_max <- angle_max
  new_list$x$state$angle_number <- angle_number
  new_list$x$state$font_size_min <- font_size_min
  new_list$x$state$font_size_max <- font_size_max
  list_size_mode <- c("linear", "sqrt", "log", NULL)
  if (!is.null(size_mode)) {
    if (!size_mode %in% list_size_mode) {
      stop("`size_mode` must be one of: c('linear', 'sqrt', 'log')")
    } else {
      if (
        length(jsonlite::fromJSON(
          '{"linear": "linear", "sqrt": "sqrt", "log": "log"}'
        )) >
          1
      ) {
        ...
        # Set the names of the list to boolean values
        names(named_list) <- boolean_names
        if (length(named_list) > 1 && size_mode %in% names(named_list)) {
          if (is.logical(size_mode)) {
            size_mode <- as.character(size_mode)
          }
          named_list_value <- unname(named_list[size_mode])[[1]]
          new_list$x$state$size_mode <- named_list_value
        }
      }
    }
  }
  ...
  if (!is.null(height_mode)) {
    if (!height_mode %in% list_height_mode) {
      ...
      if (
        length(jsonlite::fromJSON(
          '{"fill_space": "fill_space", "aspect": "aspect"}'
        )) >
          1
      ) {
        named_list <- jsonlite::fromJSON(
  ...
  list_animation_type <- c("auto", "fade", "grow", "none", NULL)
  if (!is.null(animation_type)) {
    if (!animation_type %in% list_animation_type) {
      stop("`animation_type` must be one of: c('auto', 'fade', 'grow', 'none')")
    } else {
      if (
        length(jsonlite::fromJSON(
          '{"auto": "auto", "fade": "fade", "grow": "grow", "none": "none"}'
        )) >
          1
      ) {
        named_list <- jsonlite::fromJSON(
        ...
        # Set the names of the list to boolean values
        names(named_list) <- boolean_names
        if (length(named_list) > 1 && animation_type %in% names(named_list)) {
          if (is.logical(animation_type)) {
            animation_type <- as.character(animation_type)
          }
          named_list_value <- unname(named_list[animation_type])[[1]]
          new_list$x$state$animation_type <- named_list_value
        }
      }
    }
  }
  new_list$x$state$duration <- duration
  new_list$x$state$stagger <- stagger
